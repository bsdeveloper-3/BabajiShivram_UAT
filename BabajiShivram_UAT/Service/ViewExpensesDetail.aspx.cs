...
public partial class Service_ViewExpensesDetail : System.Web.UI.Page
{
    ...
    protected void PCDDocumentSqlDataSource_Selected(object sender, SqlDataSourceStatusEventArgs e)
    {
        if (e.Exception != null)
        {
            ...
            ErrorLog.LogToDatabase(Convert.ToInt32(Session["JobIdE"]), "PCDDocumentSqlDataSource_Selected:JobView.aspx",
                PCDDocumentSqlDataSource.SelectCommand, e.Exception.Message, e.Exception, lblError.Text, LoggedInUser.glUserId);
            ErrorLog.SendMail("Error Reported In Job View:Job Detail Form For PCDDocumentSqlDataSource_Selected", e.Exception);
        }
    }
    ...
    protected void gvBillDetail_RowCommand(Object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName.ToLower() == "download")
        {
            ...
            DataSet dsDocDetail = BillingOperation.GetBillDocById(Convert.ToInt32(Session["JobIdE"]), BillId, 10);
    ...
    {
        Int32 ExpenseID = Convert.ToInt32(Session["ExpenseID"]);

        Int32 ApprovalStatusId = (Int32)AdditionalExpenseStatus.PaymentCompleted;
        string strApprovalRemark = txtAuditRemark.Text.Trim();

        if (strApprovalRemark != "")
        {
            int result = DBOperations.AddJobExpensesStatus(ExpenseID, ApprovalStatusId, strApprovalRemark, LoggedInUser.glUserId);
            ...
            if (result == 0)
            {
                ...
                Response.Redirect("AdditionalExpensePost.aspx");
            }
            else if(result == 2)
            {
                lblError.Text       =   "Expense Details Already Booked!";
                lblError.CssClass   =   "errorMsg";
            }
            else
            {
        ...
        {
            lblError.Text       =   "Please Enter Audit Remark!";
            lblError.CssClass   =   "errorMsg";
        }
    }
}
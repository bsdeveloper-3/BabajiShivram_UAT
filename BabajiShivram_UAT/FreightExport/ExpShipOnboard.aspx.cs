...
public partial class FreightExport_ExpShipOnboard : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("ExpPendingShipOnboard.aspx");
        }
        else if (!IsPostBack)
        {
    ...
    private void SetFreightDetail(int EnqId)
    {
        ...
        if (dsBookingDetail.Tables[0].Rows.Count > 0)
        {
            if (dsBookingDetail.Tables[0].Rows[0]["FRJobNo"] != DBNull.Value)
            {
                ...
                lblMBLNo.Text = dsBookingDetail.Tables[0].Rows[0]["MBLNo"].ToString();
                lblHBLNo.Text = dsBookingDetail.Tables[0].Rows[0]["HBLNo"].ToString();
                lblcontainerNo.Text = dsBookingDetail.Tables[0].Rows[0]["ContainerNo"].ToString();

                if (dsBookingDetail.Tables[0].Rows[0]["MBLDate"] != DBNull.Value)
                    lblMBLDate.Text = Convert.ToDateTime(dsBookingDetail.Tables[0].Rows[0]["MBLDate"]).ToString("dd/MM/yyyy");

                if (dsBookingDetail.Tables[0].Rows[0]["HBLDate"] != DBNull.Value)
                    lblHBLDate.Text = Convert.ToDateTime(dsBookingDetail.Tables[0].Rows[0]["HBLDate"]).ToString("dd/MM/yyyy");
            }
        }
    }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        {
            OnboardDate = Commonfunctions.CDateTime(txtOnBoardDate.Text.Trim());
        }
        else
        {
            lblError.Text = "Please Enter On Board Date!";
        ...
        int result = DBOperations.AddExportOnBoard(EnqId, OnboardDate, strRemark, LoggedInUser.glUserId);

        if (result == 0)
        {
            string strReturnMessage = "Shipped On Board Detail Updated Successfully!";

            ScriptManager.RegisterStartupScript(this, this.GetType(), "alertMessage", "alert('" + strReturnMessage + "'); window.location='ExpPendingShipOnboard.aspx';", true);
        }
        else if (result == 1)
        {
        ...
        {
            lblError.Text = "Shipped On Board  Detail Already Exists!";
            lblError.CssClass = "errorMsg";
        }
        else
        {
    ...
    protected void btnSendEmail_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (bEMailSucess == true)
            {
                ...
                if (Result == 0)
                {
                    lblError.Text = "Shipped OnBoard Email Sent Successfully!";
                    lblError.CssClass = "success";
                }
                else if (Result == 1)
                {
    ...
    private void GenerateEmailDraft()
    {
        ...
        txtSubject.Text = "Export Shipped OnBoard" + "-" + lblJobNo.Text.Trim() + " // " + lblEnquiryNo.Text.Trim() + " // " + lblCustomer.Text.Trim() + " // " + lblPortLoading.Text.Trim() + " // " + lblPortDischarge.Text.Trim();
        ...
        string VGMDate = "", Form13Date = "", MBLNo = "", HBLNo = "", MBLDate = "", HBLDate = "", OnBoardDate = "";
        ...
        if (dsAlertDetail.Tables[0].Rows.Count > 0)
        {
            ...
            MBLNo = dsAlertDetail.Tables[0].Rows[0]["MBLNo"].ToString();
            HBLNo = dsAlertDetail.Tables[0].Rows[0]["HBLNo"].ToString();

            if (dsAlertDetail.Tables[0].Rows[0]["MBLDate"] != DBNull.Value)
            {
                MBLDate = Convert.ToDateTime(dsAlertDetail.Tables[0].Rows[0]["MBLDate"]).ToString("dd/MM/yyyy");
            }
            if (dsAlertDetail.Tables[0].Rows[0]["HBLDate"] != DBNull.Value)
            {
                HBLDate = Convert.ToDateTime(dsAlertDetail.Tables[0].Rows[0]["HBLDate"]).ToString("dd/MM/yyyy");
            }

            
            if (txtOnBoardDate.Text.Trim() != "")
            {
                OnBoardDate = Convert.ToDateTime(txtOnBoardDate.Text.Trim()).ToString("dd/MM/yyyy");
            }
        ...
        try
        {
            ...
            string strFileName = "../EmailTemplate/FOP_ShipOnBoardMail.txt";
        ...
        MessageBody = MessageBody.Replace("@ShipOnboardDate", OnBoardDate);

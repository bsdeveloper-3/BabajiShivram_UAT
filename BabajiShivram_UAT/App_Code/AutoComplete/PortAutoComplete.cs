...
/// <summary>
/// Summary description for PortAutoComplete
/// </summary>
[WebService(Namespace = "http://tempuri.org/")]
[WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
// To allow this Web Service to be called from script, using ASP.NET AJAX, uncomment the following line. 
[System.Web.Script.Services.ScriptService]
public class PortAutoComplete : System.Web.Services.WebService {

    public PortAutoComplete () {
    ...
    public string[] GetCompletionList(string prefixText, int count)
    {
        ...
        cmd.CommandText = "SELECT lId AS PortId,PortName FROM BS_PortMS WHERE PortName like @myParameter AND bDel=0 ORDER BY PortName";
        ...
        List<Port> txtItems = new List<Port>();
        String dbValues;
        ...
        foreach (DataRow row in dt.Rows)
        {
            ...
            PortList.PortId = Convert.ToInt32(row["PortId"]);
            PortList.Name = row["PortName"].ToString();
            txtItems.Add(PortList);

        }

        List<string> items = new List<string>();
        foreach (Port c in txtItems)
        {

...
public partial class EWayBill_EWayBillRequest : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        CalExtDocDate.EndDate = Convert.ToDateTime(DateTime.Now.ToString("dd/MM/yyyy"));
        ...
        if (!IsPostBack)
        {
            ...
            lblTitle.Text = "E-Way Bill Generation";
            ...
            //DBOperations.FillEWAYBillGSTIN(ddTrasnporter, LoggedInUser.glModuleId);

            DBOperations.FillEWAYBillGSTIN2(ddTrasnporter, LoggedInUser.glModuleId);
    ...
    private void GetJobDetail(int JobId)
    {
        ...
        if (dsJobDetail.Tables.Count > 0)
        {
            ...
            {
                txtBOENo.Text = dsJobDetail.Tables[0].Rows[0]["BOENo"].ToString();
            }
            if (dsJobDetail.Tables[0].Rows[0]["BOEDate"] != DBNull.Value)
            {
                txtBOEDate.Text = Convert.ToDateTime(dsJobDetail.Tables[0].Rows[0]["BOEDate"]).ToString("dd/MM/yyyy");
            }
            ...
            // Bind Eway Bill History

            BindJobEwayHistory(JobId);
        }
    }
    ...
    private void BindJobEwayHistory(int JobId)
    {
        // DataSet dsHistory = DBOperations.GetTransportEwayBillByJobID(JobId);

        //DataSet dsHistory = DBOperations.GetEwayBillByJobID(JobId);

        DSEwayHistory.SelectParameters["JobId"].DefaultValue = JobId.ToString();
        gvEwayHistory.DataBind();

        if (gvEwayHistory.Rows.Count > 0)
        {
            ModalPopupHistory.Show();
        }
        else
        {
            ModalPopupHistory.Hide();
        }
    }
    ...
    protected void txtDispatchQuantity_TextChanged(object sender, EventArgs e)
    {
        ...
        if (chk1.Checked)
        {
            decimal.TryParse(txtDispatchQuantity.Text.Trim(), out decDispatch);
        ...
        {
            ShowUserAlert("Please Select Product!");
        }
    }
    ...
    private DataTable GetBalanceProduct(GridView gvBalProduct, string strVehicleNo, string strTranDocNo, ref bool IsValid)
    {
        ...
        foreach (GridViewRow gvRow in gvBalProduct.Rows)
        {
            ...
            //if (chkSelect.Checked)
            {
                ...
                drFinal["Doc No"] = txtBOENo.Text.Trim();
                drFinal["Doc Date"] = txtBOEDate.Text.Trim();
                ...
                drFinal["Trans Name"] = ddTrasnporter.SelectedItem.Text; //txtTransporterName.Text.Trim();
                drFinal["Trans ID"] = ddTrasnporter.SelectedValue; //txtTransporterGSTIN.Text.Trim();
    ...
    public string WriteJsonClass(DataRow[] itemRows, string path)
    {
        ...
        //root.version = "1.0.0201";
        ReqGenEwbPl BillList = new ReqGenEwbPl();
        try
        {
            string strVehicleNo = "";
            {
                List<ReqGenEwbPl.ItemListInReqEWBpl> itm = new List<ReqGenEwbPl.ItemListInReqEWBpl>();
                ...
                BillList.docNo          =   txtBOENo.Text.Trim();
                BillList.docDate        =   txtBOEDate.Text.Trim();
                ...
                BillList.actFromStateCode = Convert.ToInt32(ddDespatchFromState.SelectedValue);
                ...
                BillList.actToStateCode =   Convert.ToInt32(ddState.SelectedValue); // Delivery State

                BillList.transactionType =  Convert.ToInt32(ddTransactionType.SelectedValue); // Movement/Transaction Type
                ...
                BillList.transporterName =  ddTrasnporter.SelectedItem.Text;// txtTransporterName.Text.Trim();
                BillList.transporterId  =   ddTrasnporter.SelectedValue; //txtTransporterGSTIN.Text.Trim();
                BillList.transDocNo     =   "";
                ...
                foreach (DataRow dr in itemRows)
                {
                    ...
                    ReqGenEwbPl.ItemListInReqEWBpl ProductList = new ReqGenEwbPl.ItemListInReqEWBpl();
                    ...
                    strVehicleNo = dr["Vehicle No"].ToString().Trim();
                    ...
                    if (strVehicleNo != "")
                    {
                        ...
                        BillList.transDocNo = dr["Trans Doc No"].ToString();
                    }
                    ...
                    if (dr["GSTDutyRate"].ToString() != "")
                    {
                        ...
                        {
                            if (BillList.igstValue == 0)
                            {
                                // IF IGST Amount IS Zero Set Tax Rate also Zero -- IGST Tax Waiver
                                ProductList.igstRate = 0;
                            }
                            else
                            {
            ...
            var json = serializer.Serialize(BillList);
    ...
    protected void APIRequest()
    {
        ...
        Session["EwayMessageReject"] = null;
        ...
        if (ViewState["vwsFinalEway"] != null)
        {
            strAPI_GSTIN = ddTrasnporter.SelectedValue.Trim();
            ...
            int intTransType = 1; // Import
            int intTansSubType = 0;

            if(ddTransType.SelectedValue.ToUpper() == "O")// Export
            {
                intTransType = 2;
            }

            intTansSubType = Convert.ToInt32(ddSubType.SelectedValue);

            string strDocumentType, strDocumentNumber = "";
            string strUserGSTIN = ddTrasnporter.SelectedValue.Trim();

            Jobid = Convert.ToInt32(hdnJobId.Value);
            DateTime dtDocumentDate = DateTime.MinValue;

            strDocumentType = ddDocType.SelectedValue;
            strDocumentNumber = txtBOENo.Text.Trim();

            if (txtBOEDate.Text.Trim() != "")
            {
                dtDocumentDate = Commonfunctions.CDateTime(txtBOEDate.Text.Trim());
            }

            DataTable dtJson = (DataTable)ViewState["vwsFinalEway"];

            DataView viewVehicle = new DataView(dtJson);

            DataTable distinctValues = viewVehicle.ToTable(true, "VehicleID");

            if(distinctValues.Rows.Count != TotalVehicle)
            {
                lblError.Text = "Please Check Number of Vehicle Placed and Assigned Product Detail per Vehicle";
                lblError.CssClass = "errorMsg";
                                
                return;
            }
            
            List<RespGenEwbPl> EwaySuccessList = new List<RespGenEwbPl>();

            List<String> EwayErrorList = new List<string>();
            ...
            for (int i = 0; i < TotalVehicle; i++)
            {
                ...
                itemRows = dtJson.Select(expression);
                
                string strVehicleNo = itemRows[0]["Vehicle No"].ToString();
                string strVehicleDate = txtTransportDate.Text.Trim();

                string JsonPayload = WriteJsonClass(itemRows, "");

                if (JsonPayload != "")
                {
                    TxnRespWithObj<RespGenEwbPl> TxnResp = EWBAPI.GenEWBAsync(EwbSession, JsonPayload);
                    
                    if (TxnResp.IsSuccess)
                    {
                        string strBillDate = ""; string strValidityDate = "";
                        string strEWayBillNo = "";

                        EwaySuccessList.Add(TxnResp.RespObj);
                        
                        if (TxnResp.RespObj != null)
                        {
                            lblError.Text += "<BR>" + (i + 1).ToString() + ". " + " Success : EWay Bill No: " + TxnResp.RespObj.ewayBillNo;
                            lblError.CssClass = "errorMsg";
                            
                            strEWayBillNo = TxnResp.RespObj.ewayBillNo;

                            strBillDate = TxnResp.RespObj.ewayBillDate;
                            strValidityDate = TxnResp.RespObj.validUpto;
                        }

                        DBOperations.AddEWayBill(intTransType, intTansSubType, Jobid, lblJobRefNo.Text.Trim(), strEWayBillNo, strDocumentType, strDocumentNumber,
                            dtDocumentDate, strVehicleNo, strVehicleDate, strBillDate, strValidityDate, strUserGSTIN,strAPI_GSTIN, LoggedInUser.glUserId);
                        
                    }//Success
                    else
                    {
                        EwayErrorList.Add(TxnResp.TxnOutcome);

                        lblError.Text += "<BR>" + (i + 1).ToString() + ". Error : " + TxnResp.TxnOutcome;
                        lblError.CssClass = "errorMsg";
                    }

                }// END_IF_JsonPayload
                else // JSON Parsing Exception
                {
                    lblError.Text += "<BR>" + (i + 1).ToString() + ". Error Internal : " + "Invalid JSON Parse Error.";
                    lblError.CssClass = "errorMsg";
                }
            }//END_FOR
            ...
            {
                Session["EwaySuccessList"] = EwaySuccessList;

                if(EwayErrorList.Count > 0)
                {
                    Session["EwayErrorList"] = EwayErrorList;
                }
    ...
    protected void lnkPopupClose_Click(object sender, EventArgs e)
    {
        ModalPopupHistory.Hide();
    }

    protected void gvEwayHistory_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if(e.CommandName.ToLower() == "printewaybill")
        {
            Session["EwayPrint"] = e.CommandArgument.ToString(); //EwbNo;

            if (ddTrasnporter.SelectedValue == "0")
            {
                Session["userGSTIN"] = ddTrasnporter.Items[1].Value;
            }
            else
            {
                Session["userGSTIN"] = ddTrasnporter.SelectedItem.Value;
            }

...
using System.IO;
public partial class Freight_FreightMISProfitCustomer : System.Web.UI.Page
{
    ...
    protected void gvProfit_RowCommand(Object Sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName.ToLower() == "select")
        {
            ...
            string strCustomerID = gvProfit.DataKeys[rowIndex].Value.ToString();


            BindProfitDetail(strCustomerID, strReportMonth);
        }
    }

    private void BindProfitDetail(string CustId, string MonthID)
    {
        pnlProfitDetailXLS.Visible = false;

        if (Convert.ToInt32(MonthID) >= 0)
        {
            ProfitDetailSqlDataSource.SelectParameters["CustId"].DefaultValue = CustId;
    ...
    {
        string strFileName = "Freight_Customer_Profit" + DateTime.Now.ToString("dd/MM/yyyy hh:mm tt") + ".xls";
    ...
    {
        string strFileName = "Freight_Customer_Buy-Sell-Detail" + DateTime.Now.ToString("dd/MM/yyyy hh:mm tt") + ".xls";
        string header = "attachment;filename=" + strFileName;
        string contentType = "application/vnd.ms-excel";

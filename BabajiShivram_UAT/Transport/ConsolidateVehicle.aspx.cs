...
public partial class Transport_ConsolidateVehicle : System.Web.UI.Page
{
    ...
    protected void btnSaveRate_Click(object sender, EventArgs e)
    {
        ...
        decimal dcAdvance = 0, dcRate = 0, dcAdvanceAmount = 0, dcFreightAmount = 0, dcVaraExp = 0, dcDetentionCharges = 0, dcEmptyContRecptCharges = 0,
                dcTollCharges = 0, dcOtherCharges = 0, dcMarketRate = 0;
                ...
                if (gvTransportJobDetail != null && gvTransportJobDetail.Rows.Count > 0)
                {
                    for (int i = 0; i < gvTransportJobDetail.Rows.Count; i++)
                    {
                        if (gvTransportJobDetail.DataKeys[i].Value.ToString() != "")
                        {
                            ...
                            int result = DBOperations.UpdateTransportRateDetail(TransReqId, TransporterId, VehicleType, VehicleId, VehicleNo, MemoFilePath, txtCity.Text.Trim(), dcMarketRate, "",
                                                "", dcRate, dcAdvance, dcAdvanceAmount, dcFreightAmount, dcDetentionCharges, dcVaraExp,
                                                dcEmptyContRecptCharges, dcTollCharges, dcOtherCharges, LRDate, ChallanDate, 0, 0, "", "", LoggedInUser.glUserId);
                            if (result == 0)
                            {
                                if (txtBillingInstruction.Text != "")
                                {
                                    int AddInstruction = DBOperations.TR_AddBillingInstructions(TransReqId, TransReqId, txtBillingInstruction.Text.Trim(), LoggedInUser.glUserId);
                                }
                    ...
                    for (int i = 0; i < gvTransportJobDetail.Rows.Count; i++)
                    {
                        if (gvTransportJobDetail.DataKeys[i].Value.ToString() != "")
                        {
                            ...
                            int result = DBOperations.AddTransportRateDetail(true, TransReqId, VehicleId, TransporterId, VehicleType, VehicleNo, MemoFilePath, txtCity.Text.Trim(), dcMarketRate, "",
                                "", dcRate, dcAdvance, dcAdvanceAmount, dcFreightAmount, dcDetentionCharges, dcVaraExp,
                                dcEmptyContRecptCharges, dcTollCharges, dcOtherCharges, LRDate, ChallanDate, 1, LoggedInUser.glUserId);
                            if (result > 2)
                            {
                                int AddTP = DBOperations.AddTransporterPlaced(TransReqId, TransporterId, LoggedInUser.glUserId);
                                if (txtBillingInstruction.Text != "")
                                {
                                    int AddInstruction = DBOperations.TR_AddBillingInstructions(TransReqId, result, txtBillingInstruction.Text.Trim(), LoggedInUser.glUserId);
                                }
        ...
        try
        {
            if (PaymentId != 0)
            {
                ...
                if (dsGetPaymentDetails != null)
                {
                    ...
                    strCCEmail = "ho.accounts7@babajishivram.com, ho.accounts9@babajishivram.com , " + CreatedByEmail; 

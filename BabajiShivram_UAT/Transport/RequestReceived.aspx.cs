...
public partial class Transport_RequestReceived : System.Web.UI.Page
{
    ...
    protected void gvJobDetail_RowCommand(Object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            Response.Redirect("VehiclePlace.aspx");
        }
    }
    ...
    #region Vehicle Place
    protected void btnVehiclePlace_Click(object sender, EventArgs e)
    {
        lblMessage.Text = "";
        ...
        foreach (GridViewRow row in gvJobDetail.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                {
                    lblMessage.Text = "";
                    string TransReqId = gvJobDetail.DataKeys[row.RowIndex].Value.ToString();
                    Session["TransReqId"] = TransReqId;
                    DBOperations.FillCompanyByCategory(ddtransporterName, Convert.ToInt32(EnumCompanyType.Transporter));
                    mpeVehiclePlaced.Show();
                    break;
                }
                else
                {
                    lblMessage.Text = "Please select Job No";
                    lblMessage.CssClass = "errorMsg";
                }
            }
        }
    ...
    protected void btnAddVehiclePlaced_Click(object sender, EventArgs e)
    {
        int NoOfVehicle = Convert.ToInt32(txtNoOfVehicle.Text);
        int TransporterId = Convert.ToInt32(ddtransporterName.SelectedValue);

        int result = DBOperations.Tr_InsVehiclePlacedDetail(Convert.ToInt32(Session["TransReqId"]), NoOfVehicle, TransporterId, LoggedInUser.glUserId);
        if (result == 0)
        {
            lblMessage.Text = "Success";
            lblMessage.CssClass = "errorMsg";
            mpeVehiclePlaced.Hide();
            gvJobDetail.DataBind();
        }

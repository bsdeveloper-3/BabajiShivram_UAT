...
public partial class Transport_ApprovedBillByDept : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
       // ScriptManager1.RegisterPostBackControl(lnkexport);
        ScriptManager1.RegisterPostBackControl(gvVendorJobDetail);
        if (!IsPostBack)
        {
            if (Convert.ToString(Session["UserId"]) != null)
            {
                Label lblTitle = (Label)Page.Master.FindControl("lblTitle");
                lblTitle.Text = "Approved Transporter Bill";
            }
        }

        DataFilter1.DataSource = DataSourceVendorJobs;
        DataFilter1.DataColumns = gvVendorJobDetail.Columns;
        DataFilter1.FilterSessionID = "ApprovedBillByDept.aspx";
    ...
    protected void gvVendorJobDetail_RowCommand(Object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName.ToLower() == "selectvehicles")
        {
            ...
            string strJobId = gvVendorJobDetail.DataKeys[gvrow.RowIndex].Values[0].ToString();
            string strTransporterId = gvVendorJobDetail.DataKeys[gvrow.RowIndex].Values[1].ToString();
            string strVehicleNo = gvVendorJobDetail.DataKeys[gvrow.RowIndex].Values[2].ToString();
            ...
            if (strJobId != "")
            {
                ...
                Response.Redirect("VehicleBillDetailsFinal.aspx");
            }
        }
        ...
        //else if (e.CommandName.ToLower() == "approveBill")
        //{
        //    GridViewRow gvrow = (GridViewRow)((LinkButton)e.CommandSource).NamingContainer;
        //    string JobId = gvVendorJobDetail.DataKeys[gvrow.RowIndex].Value.ToString();

        //    if (JobId != "")
        //    {
        //        int approvedResult = DBOperations.UpdateJobBillStatus(Convert.ToInt32(JobId.ToString()), Convert.ToInt32(1), Convert.ToInt32(LoggedInUser.glUserId),
        //                            Convert.ToInt32(LoggedInUser.glUserId), Convert.ToDateTime(DateTime.Now), Convert.ToString(""));
        //        if (approvedResult == 0)
        //        {
        //            gvVendorJobDetail.DataBind();
        //            lblError.Text = "Successfully approved bill status. Job moved to Approved By Bill Department tab.";

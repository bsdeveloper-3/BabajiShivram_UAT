...
public partial class Transport_EWayBill : System.Web.UI.Page
{
    ...
    {        
        ...
        if (!IsPostBack)
        {
            ...
            ddDespatchFromState.Items[0].Text = "-Select State-";
        }
    ...
    private void GetJobDetail(int JobId)
    {
        ...
        if (dsJobDetail.Tables.Count > 0)
        {
            ...
            {
                lblSupplier.Text = dsJobDetail.Tables[0].Rows[0]["Supplier"].ToString();
            }
            if (dsJobDetail.Tables[0].Rows[0]["Consignee"] != DBNull.Value)
            {
                lblConsignee.Text = dsJobDetail.Tables[0].Rows[0]["Consignee"].ToString();
            }
            if (dsJobDetail.Tables[0].Rows[0]["ConsigneeGSTIN"] != DBNull.Value)
            {
                lblConsigneeGSTIN.Text = dsJobDetail.Tables[0].Rows[0]["ConsigneeGSTIN"].ToString();
            }
    ...
    private DataTable GetBalanceProduct(GridView gvBalProduct, string strVehicleNo, string strTranDocNo, ref bool IsValid)
    {
        ...
        foreach(GridViewRow gvRow in gvBalProduct.Rows)
        {
            ...
            //if (chkSelect.Checked)
            {
                ...
                drFinal["Frm_OtherPartyName"] = lblSupplier.Text.Trim();
                drFinal["From_GSTIN"]    =  "URP";
                drFinal["From_Address1"] =  "";
                drFinal["From_Address2"] =  "";

                drFinal["From_Place"]   =   "";
                drFinal["From_PinCode"] =   "999999";
                drFinal["From_State"]   =   "Other Countries";
                drFinal["TO_GSTIN"]     =   lblConsigneeGSTIN.Text.Trim();
                ...
                drFinal["To_State"]     =   ddState.SelectedItem.Text;
    ...
    public string WriteJsonClass(DataTable dtEWay, string path)
    {
        ...
        try
        {
            for (int i = 0; i < TotalVehicle; i++)
            {
                ...
                BillList.subSupplyType  =   Convert.ToInt16(ddSubType.SelectedValue);
                BillList.docType        =   ddDocType.SelectedValue;
                BillList.docNo          =   lblBOENo.Text.Trim();
                BillList.docDate        =   lblBOEDate.Text.Trim();

                BillList.fromGstin      =   "URP";
                BillList.fromTrdName    =   lblSupplier.Text.Trim();
                BillList.fromAddr1      =   "";
                BillList.fromAddr2      =   "";
                BillList.fromPlace      =   "";
                BillList.fromPincode    =   999999;
                BillList.fromStateCode  =   99;
                BillList.actualFromStateCode = Convert.ToInt32(ddDespatchFromState.SelectedValue);
                
                BillList.toGstin        =   lblConsigneeGSTIN.Text.Trim();
                BillList.toTrdName      =   lblConsignee.Text.Trim();
                ...
                BillList.toStateCode    =   Convert.ToInt32(ddState.SelectedValue);
                BillList.actualToStateCode = Convert.ToInt32(lblConsigneeGSTIN.Text.Trim().Substring(0, 2));

...
using System.Text;
using Ionic.Zip;
public partial class Transport_NewTransVehicleDetail : System.Web.UI.Page
{
    ...
    protected void gvRateDetail_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                if (DataBinder.Eval(e.Row.DataItem, "JobDeliveryVehicleNo") != "")
                {
        ...
        if (ExpenseTypeId != 0) // && BranchId != 0
        {
            ...
            if (count == 0)
            {
                ...
                result = DBOperations.AddTransFunRequest(JobId, JobRefNo, TranRequestId, TransporterId, ExpenseTypeId, PaymentTypeId, dcAmount, BranchId,
                            txtRemark.Text.Trim(), AdvanceRecd, dcAdvanceAmount, dcAmount.ToString(), 3, LoggedInUser.glUserId);
                ...
                {
                    lblFundError.Text = "Payment Request already exists for Job Number and Expense Type and Amount.";
                    lblFundError.CssClass = "errorMsg";
                }
                else if (result == 3)
                {
        ...
        try
        {
            if (PaymentId != 0)
            {
                ...
                if (dsGetPaymentDetails != null)
                {
                    ...
                    strCCEmail = "javed.shaikh@babajishivram.com, ho.accounts7@babajishivram.com, ho.accounts9@babajishivram.com, bsameer@babajishivram.com , " + CreatedByEmail;

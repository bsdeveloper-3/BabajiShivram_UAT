...
using System.Text;
using Ionic.Zip;
public partial class Transport_VehiclePlace : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //mevLrDate.MaximumValue = DateTime.Now.ToString("dd/MM/yyyy");
    ...
    protected void lnkbtnFundRequest_Click(object sender, EventArgs e)
    {
       ...
       // txtJobNumber_TextChanged(null, EventArgs.Empty);
        txtAmount.Text = row.Cells[8].Text.Trim();
    ...
    protected void btnSaveRate_Click(object sender, EventArgs e)
    {
        ...
        decimal dcAdvance = 0, dcRate = 0, dcAdvanceAmount = 0, dcFreightAmount = 0, dcVaraExp = 0, dcDetentionCharges = 0, dcEmptyContRecptCharges = 0,
                dcTollCharges = 0, dcOtherCharges = 0, dcMarketRate = 0, dcContractPrice = 0, dcSellingPrice = 0;//2020
        ...
        if (Session["TRId"] != null)
        {
            ...
            string MemoFilePath = "", EmailFilePath = "", ContractFilePath = "";//2020
            ...
            if (hdnIsUpdate.Value == "1")
            {
                ...
                {
                    int result = DBOperations.UpdateTransportRateDetail(Convert.ToInt32(hdnLid.Value), TransporterId, VehicleType, VehicleId, VehicleNo, MemoFilePath, txtCity.Text.Trim(), dcMarketRate, "",
                    ...
                    if (result == 0)
                    {
                        int AddTP1 = DBOperations.AddTransporterPlaced(TransReqId, TransporterId, LoggedInUser.glUserId);

                        if (txtBillingInstruction.Text != "")
                        {
                            int AddInstruction = DBOperations.TR_AddBillingInstructions(TransReqId, Convert.ToInt32(hdnLid.Value), TransporterId, txtBillingInstruction.Text.Trim(), LoggedInUser.glUserId);
                        }
                    ...
                    {
                        lblError.Text = "Advace requested, cant change amount";
                        lblError.CssClass = "errorMsg";
                    }
                    else
                    {
            ...
            {
                int result = DBOperations.AddTransportRateDetail(false, TransReqId, VehicleId, TransporterId, VehicleType, VehicleNo, MemoFilePath, txtCity.Text.Trim(), dcMarketRate, "",
                                            "", dcRate, dcAdvance, dcAdvanceAmount, dcFreightAmount, dcDetentionCharges, dcVaraExp,
                                            dcEmptyContRecptCharges, dcTollCharges, dcOtherCharges, LRDate, ChallanDate, DispatchedCount,
    ...
    protected void TruckRequestDetail(int TransportId)
    {
        ...
        {
            int CompanyId = Convert.ToInt32(dvDetail.Table.Rows[0]["CompanyID"]);
            if (CompanyId == 3)
            {
                btnFundRequest.Visible = false;
                btnFundRequest.Enabled = false;
            }
            else
            {
                btnFundRequest.Visible = true;
                btnFundRequest.Enabled = true;
            }
            ...
            hdnJobRefNo.Value = lblJobNo.Text;
            ...
            lblRemark.Text = dvDetail.Table.Rows[0]["Remark"].ToString();
            lblPickAdd.Text = dvDetail.Table.Rows[0]["PickUpAddress"].ToString();
            lblDropAdd.Text = dvDetail.Table.Rows[0]["DropAddress"].ToString();
            lblpickPincode.Text = dvDetail.Table.Rows[0]["PickupPincode"].ToString();                                    //added new pickup and drop pincode city and state for truck request Recieved 
            ...
            hdnFilePath.Value = strCustDocFolder + strJobFileDir;
            hdnJobId.Value = dvDetail.Table.Rows[0]["JobId"].ToString();
            Session["BillingAdvice"] = dvDetail.Table.Rows[0]["PCDToBillingAdvice"].ToString();
            gvRateDetail.DataBind();
        }
    }
    ...
    protected void gvRateDetail_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName.ToLower().Trim() == "edit")
        {
            ...
            if (lid > 0)
            {
                ...
                if (dvGetRateDetail != null)
                {
                    ...
                    if (ddlTransporter.SelectedValue == "524" || ddlTransporter.SelectedValue == "17304") // Navbharat/NavJeevan
                    {
                    ...
                    txtContractPrice.Text = dvGetRateDetail.Table.Rows[0]["ContractPrice"].ToString();                  // Changes For Selling Frieght Rate
        ...
        else if (e.CommandName.ToLower().Trim() == "deleterow")
        {
            ...
            if (lid > 0)
            {
                ...
                {
                    lblError.Text = "Error: Advance Fund Already Requested!";
                    lblError.CssClass = "errorMsg";
                }
                else if (result == 4)
                {
                    lblError.Text = "Error: Vehicle Already Delivered!";
                    lblError.CssClass = "errorMsg";
                }
                else
                {
    ...
    protected void txtJobNumber_TextChanged(object sender, EventArgs e)
    {
        ...
        if (txtJobNumber.Text.Trim() != "")
        {
            if (hdnJobId.Value != "0")
            {
                DataSet dsGetJobDetail = AccountExpense.GetJobdetailById(Convert.ToInt32(hdnJobId.Value), 3);
        ...
        if (ExpenseTypeId != 0) // && BranchId != 0
        {
            ...
            if (count == 0)
            {
                ...
                if (result != 1 && result != 2 && result != 3)
                {
                    ...
                    lblError.Text = "Advance Request Sent For Management Approval!";
                    lblError.CssClass = "success";
        ...
        try
        {
            if (PaymentId != 0)
            {
                ...
                if (dsGetPaymentDetails != null)
                {
                    ...
                    strCustomerEmail = "neeraj.mandowara@babajishivram.com ,  ho.accounts7@babajishivram.com"; // aman.jain@babajishivram.com ,jr.developer@babajishivram.com";
                    ...
                    strCCEmail = "javed.shaikh@babajishivram.com , ho.accounts7@babajishivram.com , ho.accounts9@babajishivram.com , bsameer@babajishivram.com , " + CreatedByEmail;    //"javed.shaikh@babajishivram.com, " + CreatedByEmail + ", developer@babajishivram.com";
    ...
    protected void btnFundRequest_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in gvRateDetail.Rows)
        {
            ...
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                DataView dtRateDetail = DBOperations.GetTransRateDetailById(TransRateId);
                DataTable dt = dtRateDetail.ToTable();
                if (dt.Rows.Count > 0)
                {
                    if (chkSelect.Checked)
                    {
                        ...
                        if (count == 1)
                        {
                            ...
                            //if (Convert.ToInt32(((HiddenField)row.FindControl("hdnTransporterId")).Value.Trim()) != 0)
                            //{
                            //    ActualTransporterId = Convert.ToInt32(((HiddenField)row.FindControl("hdnTransporterId")).Value.Trim());//  Convert.ToInt32(((HiddenField)row.FindControl("hdnTransporterId")).Value.Trim());
                            //    hdnTransporterId.Value = ActualTransporterId.ToString();
                            //}

                            ActualTransporterId = Convert.ToInt32(dtRateDetail.Table.Rows[0]["TransporterId"].ToString()); 
                            hdnTransporterId.Value = ActualTransporterId.ToString();
                            ...
                            if (dcAmount > 0)
                            {
                                dcAmount = dcAmount + Convert.ToDecimal(row.Cells[17].Text.Trim());
                            }
                            else
                            {
                                dcAmount = Convert.ToDecimal(row.Cells[17].Text.Trim());
                            }
                            if (dcAmount == 0)
                            {
                                btnSubmit.Visible = false;
                                lblFundError.Text = "Invalid advance amount";
                                lblFundError.CssClass = "errorMsg";
                            }
                        ...
                        if (count > 1)
                        {
                            ...
                            if (ActualTransporterId == TransporterId)
                            {
                                ...
                                #region Get existing data for fund request if present
                                int FundRequestId = Convert.ToInt32(dtRateDetail.Table.Rows[0]["FundRequestId"].ToString());// Convert.ToInt32(((HiddenField)row.FindControl("hdnFundReqId")).Value.Trim());
                                DataSet dsGetFundDetails = AccountExpense.GetPaymentRequestById(FundRequestId);
                                if (dsGetFundDetails != null && dsGetFundDetails.Tables.Count > 0)
                                {
                                    if (dsGetFundDetails.Tables[0].Rows.Count > 0)
        ...
        if (Session["BillingAdvice"].ToString() != "1")
        {
            if (count == 0)
            {
                lblError_RateDetail.Text = "Please select atleast one vehicle rate to send fund request for!";
        ...
        {
            lblError.Text = "Vehicle already moved to billing, you can not allow fund request ";
            lblError.CssClass = "errorMsg";
        }
    }
    ...
    private void DownloadDocs(string DocumentPath)
    {

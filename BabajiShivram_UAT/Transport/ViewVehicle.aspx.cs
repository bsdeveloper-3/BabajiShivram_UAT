...
public partial class Transport_ViewVehicle : System.Web.UI.Page
{
    ...
    {
        string VehicleNo = "";
        decimal dcAdvance = 0, dcRate = 0, dcAdvanceAmount = 0, dcFreightAmount = 0, dcVaraExp = 0, dcDetentionCharges = 0, dcEmptyContRecptCharges = 0, dcMarketRate = 0;
        int TransReqId = 0, TransporterId = 0, VehicleType = 0, DispatchedCount = 0;
        ...
        if (Session["TRId"] != null)
        {
            ...
            if (ddlTransporter.SelectedValue == "524") // NAV BHARAT TRANSPORTER
            {
            ...
            if (hdnIsUpdate.Value == "1")
            {
                ...
                {
                    int result = 0; //DBOperations.UpdateTransportRateDetail(Convert.ToInt32(hdnLid.Value), TransporterId, VehicleType, VehicleNo, MemoFilePath, dcMarketRate, "",
                                    //            "", dcRate, dcAdvance, dcAdvanceAmount, dcFreightAmount, dcDetentionCharges, dcVaraExp,
                                    //            dcEmptyContRecptCharges, LRDate, ChallanDate, LoggedInUser.glUserId);
                    if (result == 0)
                    {
            ...
            {
                int result = 0; // DBOperations.AddTransportRateDetail(false, TransReqId, TransporterId, VehicleType, VehicleNo, MemoFilePath, dcMarketRate, "",
                                //            "", dcRate, dcAdvance, dcAdvanceAmount, dcFreightAmount, dcDetentionCharges, dcVaraExp,
                                //            dcEmptyContRecptCharges, LRDate, ChallanDate, DispatchedCount, LoggedInUser.glUserId);
                if (result == 0)
                {
        ...
        try
        {
            if (PaymentId != 0)
            {
                ...
                if (dsGetPaymentDetails != null)
                {
                    ...
                    {
                        strCustomerEmail = "jr.developer@babajishivram.com";    // "bsavita@babajishivram.com";         // jr.developer@babajishivram.com";
                    }
                    else
                    {
                        strCustomerEmail = "jr.developer@babajishivram.com";    //"neeraj.mandowara@babajishivram.com"; // aman.jain@babajishivram.com ,jr.developer@babajishivram.com";
                    }

                    strCCEmail = "";    //"javed.shaikh@babajishivram.com, " + CreatedByEmail + ", developer@babajishivram.com";
            ...
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkSelect.Checked)
                {
                    ...
                    if (count == 1)
                    {
                        ...
                        if (dcFreightRate > 0)
                        {
                            dcFreightRate = dcFreightRate + Convert.ToDecimal(row.Cells[13].Text.Trim());
                        }
                        else
                        {
                            dcFreightRate = Convert.ToDecimal(row.Cells[13].Text.Trim());
                        }

                        if (dcAmount > 0)
                        {
                            dcAmount = dcAmount + Convert.ToDecimal(row.Cells[15].Text.Trim());
                        }
                        else
                        {
                            dcAmount = Convert.ToDecimal(row.Cells[15].Text.Trim());
                        }

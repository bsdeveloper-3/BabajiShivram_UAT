...
public partial class Transport_VehicleBillDetails : System.Web.UI.Page
{
    ...
    protected void btnApprove_OnClick(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Convert.ToString(Session["TpJobId"]) != null && Convert.ToString(Session["TpJobId"]) != "")
            {
                ...
                {
                    if (Convert.ToString(Session["PageRedirect"]).Trim() == "ViewBill")
                    {
                        approvedResult = DBOperations.UpdateJobBillStatus(Convert.ToInt32(Session["TpJobId"].ToString()), Convert.ToInt32(1), Convert.ToInt32(LoggedInUser.glUserId),
                                 Convert.ToInt32(LoggedInUser.glUserId), Convert.ToDateTime(DateTime.Now), Convert.ToString(""), Convert.ToString(""));
                    }
                    else if (Convert.ToString(Session["PageRedirect"]).Trim() == "DeptApprovedBill")
                    {
                        approvedResult = DBOperations.UpdateJobBillStatus(Convert.ToInt32(Session["TpJobId"].ToString()), Convert.ToInt32(3), Convert.ToInt32(LoggedInUser.glUserId),
                               Convert.ToInt32(LoggedInUser.glUserId), Convert.ToDateTime(DateTime.Now), Convert.ToString(""), Convert.ToString(""));
                    }
                }
    ...
    protected void btnRejectBill_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                // (+) Added on : 28-12-2016
                // (+) Purpose  : To reject a job vehicle no instead of rejecting all vehicles of a job
                if (chklstVehicles != null && chklstVehicles.Items.Count > 0)
                {
                    for (int i = 0; i < chklstVehicles.Items.Count; i++)
                    {
                        if (chklstVehicles.Items[i].Selected == true)
                        {
                            if (chklstVehicles.Items[i].Value != null && chklstVehicles.Items[i].Value != "")
                            {
                                int RejectVehicle = DBOperations.UpdateRejectedVehicleStatus(Convert.ToInt32(chklstVehicles.Items[i].Value));
                            }
                        }
                    }
                }
                ...
                if (approvedResult == 0)
                {
                    ...
                    fsApprovedRate.Visible = false;
                    fsNform.Visible = false;
                    lblError.Text = "Successfully rejected bill.";
                    lblError.CssClass = "success";
                }
                else if (approvedResult == 1)
                {
    ...
    protected void gvVehicleDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (DataBinder.Eval(e.Row.DataItem, "ApprovalStatus") != DBNull.Value)
            {
                int Status = Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "ApprovalStatus"));
                if (Status == 0)
                {

...
public partial class ExportCHA_EditExpJobDetail : System.Web.UI.Page
{
        ...
        if (dsJobDetail.Tables[0].Rows.Count > 0)
        {
            ...
            if (dsJobDetail.Tables[0].Rows.Count > 0)
            {
                ...
                if (dsJobDetail.Tables[0].Rows[0]["ActivityStatus"] != DBNull.Value && dsJobDetail.Tables[0].Rows[0]["ActivityStatus"].ToString() != "0")
                {
                    ...
                    Session["ActivityStatus"] = dsJobDetail.Tables[0].Rows[0]["ActivityStatus"].ToString();//03-09-2020
                }
            }
        }
    ...
    private void GetExamineDetail(int JobId)
    {
        //DataSet dsJobDetail = DBOperations.GetJobDetail(JobId);
        //if (dsJobDetail.Tables[0].Rows.Count > 0)
        //{
        //    FVExamine.DataSource = dsJobDetail;
        //    FVExamine.DataBind();

        //    bool SVBApplicable = Convert.ToBoolean(dsJobDetail.Tables[0].Rows[0]["SVBApplicable"]);
        //    //Customer  SVBApplicable - Delivery Planning
        //    if (FVExamine.CurrentMode == FormViewMode.ReadOnly)
        //    {                
        //        if (SVBApplicable == true)
        //        {
        //            Panel pnlViewSVB = (Panel)FVExamine.FindControl("pnlViewSVBApplicable");
        //            pnlViewSVB.Visible = true;

        //        }
        //    }
        //    else if (FVExamine.CurrentMode == FormViewMode.Edit)
        //    {   
        //        if (SVBApplicable == true)
        //        {
        //            Panel pnlEditSVB = (Panel)FVExamine.FindControl("pnlEditSVBApplicable");
        //            pnlEditSVB.Visible = true;
    ...
    private void GetBondDetail(int JobId)
    {
        //DataSet dsBondDetail = DBOperations.GetPassingBondDetail(JobId);
        //if (dsBondDetail.Tables[0].Rows.Count > 0)
        //{
        //    int BOEType = Convert.ToInt32(dsBondDetail.Tables[0].Rows[0]["BoeType"]);

        //    fvBondDetail.DataSource = dsBondDetail;
        //    fvBondDetail.DataBind();

        //    fieldsetBond.Visible = true;
        //}
    }
    ...
    protected void FVJobDetail_DataBound(object sender, EventArgs e)
    {
        ...
        if (FVJobDetail.CurrentMode == FormViewMode.Edit)
        {
            ...
            if (drv["ContainerLoadedId"] != DBNull.Value)
            {
                DropDownList ddlContainerLoaded = (DropDownList)FVJobDetail.FindControl("ddlContainerLoaded");
                ddlContainerLoaded.SelectedValue = drv["ContainerLoadedId"].ToString();
            }
    ...
    protected void btnUpdateJob_Click(object sender, EventArgs e)
    {
        ...
        if (ddlCancelAllow.SelectedValue == "1")
        {
            ...
            else
            {
                ...
                Label lblMsg = (Label)FVJobDetail.FindControl("lblMsg");
                lblMsg.Text = "Please enter cancel Remark and select cancel Reason";
    ...
    protected void UpdateJobDetail()
    {
        ...
        bool IsADC = false; bool IsHaze = false;
        ...
        if (JobId > 0)
        {
            ...
            IsADC = ((CheckBox)FVJobDetail.FindControl("chkADC")).Checked;
            IsHaze = ((CheckBox)FVJobDetail.FindControl("chkHaze")).Checked;
            ...
            int result = EXOperations.EX_UpdateExportJobAdmin(JobId, BabajiBranchId, strCustRefNo, CustomerId, ShipperId, DivisionId, PlantId,
                ...
                IsOctroi, IsSForm, IsNForm, IsRoadPermit, PickupPersonName, dtPickUpDate, PickupMobileNo, IsADC, IsHaze, JobRemark);
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        if (txtReceiptDate.Text.Trim() != "")
        {
            ...
            {
                lblError.Text = "Invalid Receipt Date!";
                lblError.CssClass = "errorMsg";
                return;
            }
        }
        ...
        int outVal = 1;// DBOperations.AddJobExpensesDetails(JobId, ExpenseType, eAmount, reciepyNo, paidTo, ReceiptType,
            //Location, PaymentType, ReceiptAmount, dtReceiptDate, ChequeNo, dtChequeDate, IsBillable, "", LoggedInUser.glUserId);

        if (outVal == 0)
        {
            lblError.Text = "Expense Detail Added Successfully";
            lblError.CssClass = "success";
        ...
        {
            lblError.Text = "Expense Detail Already Exist.";
            lblError.CssClass = "errorMsg";
        }
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        int outVal = 1;// DBOperations.UpdateJobExpensesDetails(ExpenseId, ExpenseType, eAmount, reciepyNo, paidTo, ReceiptType,
                 // Location, PaymentType, ReceiptAmount, dtReceiptDate, ChequeNo, dtChequeDate, IsBillable, "", LoggedInUser.glUserId);

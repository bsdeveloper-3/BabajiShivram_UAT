...
public partial class PCA_PCDDraftInvoiceDetail : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            lblMsgforReceived.Text = "No Job Found For Received file for Bill Draft!";
    ...
    {
        string strFileName = "DraftInvoice_nonreceiveJoblist" + DateTime.Now.ToString("dd/MM/yyyy hh:mm tt") + ".xls";
    ...
    protected void gvNonRecievedJobDetail_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            e.Row.Cells[9].ToolTip = "Today – FSD"; //aging2
            ...
            if (DataBinder.Eval(e.Row.DataItem, "JobType") != DBNull.Value)
            {
                ...
                {
                    e.Row.Cells[10].ToolTip = "Today – Job came in Additional Tab.";
                    e.Row.Cells[6].Text = "";
                    e.Row.Cells[7].Text = "";
                }
                else
    ...
    protected void btnReceive_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow gvr in gvNonRecievedJobDetail.Rows)
        {
            if (((CheckBox)gvr.FindControl("Chk1")).Checked)
            {
                ...
                // move job to further billing stage i.e., draft check
                Session["JobRefNo"] = Recv.Text;
                GetModuleId();
                // move job to further billing stage i.e., draft check
                int Result = BillingOperation.receivedfile(Convert.ToInt32(JobId), LoggedInUser.glUserId, Convert.ToInt16(EnumBilltype.DraftInvoice), ModuleId);
                Session["JobRefNo"] = "";
                ModuleId = 0;
                ...
                string JobType = gvr.Cells[5].Text.Trim();
                string OnlineBill = gvr.Cells[12].Text.Trim();
                if (OnlineBill == "")
                {
                    if (JobType != "" && JobType.ToLower().Trim().ToString() == "additional")
                    {
                        // move job directly to final typing stage
                        int SaveDraftCheck = DBOperations.AddDraftInvoiceToTyping(Convert.ToInt32(JobId), LoggedInUser.glUserId);
                        if (SaveDraftCheck == 0)
                        {
                            lblreceivemsg.Text = "File Moved To Final Invoice Typing!!";
                            lblreceivemsg.CssClass = "success";
                        }
                        else if (SaveDraftCheck == 1)
                        {
                            lblreceivemsg.Text = "System Error! Please try after sometime!";
                            lblreceivemsg.CssClass = "errorMsg";
                        }
    ...
    protected void BtnConsolidated_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow gvr in gvNonRecievedJobDetail.Rows)
        {
            ...
            if (((CheckBox)gvr.FindControl("Chk1")).Checked)
            {
                if (i > 0)
                {
                    if (Customer != gvNonRecievedJobDetail.Rows[gvr.RowIndex].Cells[3].Text)
                    {
                ...
                Customer = gvNonRecievedJobDetail.Rows[gvr.RowIndex].Cells[3].Text;
    ...
    {
        string strFileName = "DraftInvoice_ReceiveJoblist" + DateTime.Now.ToString("dd/MM/yyyy hh:mm tt") + ".xls";
    ...
    protected void gvRecievedJobDetail_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                e.Row.Cells[9].ToolTip = "Today – FSB"; //aging1                
                e.Row.Cells[10].ToolTip = "Today – FRD"; //aging2
                e.Row.Cells[11].ToolTip = "Today – Clearance Date";//aging3
                e.Row.Cells[12].CssClass = "hidden"; //rulename
            }
    ...
    protected void gvRecievedJobDetail_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName.ToLower() == "reject" && e.CommandArgument != null)
        {
            ...
            LinkButton lnkReject = sender as LinkButton;
        ...
        if (e.CommandName.ToLower() == "fa" && e.CommandArgument != null)
        {
            ...
            Session["status"] = "FA";
            bool bApprove = true;
            result = BillingOperation.ApprovebyFADrafting(Convert.ToInt32(ViewState["JobId"]), bApprove, "", LoggedInUser.glUserId, 0, "");

            if (result == 0)
            {
                lblMsgforApproveReject.Text = "Bill Draft Approved! Job Moved Back To Draft Check!.";
            ...
            {
                lblMsgforApproveReject.Text = "Bill Draft Checked Already Pending!";
                lblMsgforApproveReject.CssClass = "errorMsg";
    ...
    protected void rpReason_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        if (chkReasonType != null && RFVRejectReason != null)
        {
            ...
            //chkReasonType.Attributes.Add("OnClick", "javascript:toggleDiv('" + chkReasonType.ClientID + "','" + lblReason.ClientID + "','" + txtReason.ClientID + "','" + lbltype.ClientID + "','" + Drplrdctype.ClientID + "','" + RFVRejectReason.ClientID + "','" + chkReasonType.Text + "','" + txtUser.ClientID + "','" + hdnUserId.ClientID + "','" + lblMailTo.ClientID + "');");
            ...
            for (int i = 0; i < rpReason.Items.Count; i++)
            {
                ...
                if (chk.Checked)
                {
                    ...
                    if (checkedCount != 0)
                    {
                        ...
                        GetModuleId();
                        result = BillingOperation.RejectDrafting(Convert.ToInt32(ViewState["JobId"]), bApprove, Remark, Convert.ToInt16(reasonforPendency), LRDC_Type, CategotyId, LoggedInUser.glUserId, ModuleId);
                        Session["JobRefNo"] = "";
                        ...
                        if (result == 0)
                        {
                            ...
                            lblMsgforApproveReject.Text = "Bill Draft Rejected! Job Moved Back To Bill Rejected!.";
                            lblMsgforApproveReject.CssClass = "success";
                        }
                        else if (result == 1)
                        {
    ...
    protected void btnSaveDocument_Click(object sender, EventArgs e)
    {
        ...
        foreach (RepeaterItem itm in rptDocument.Items)
        {
            ...
            if (chk.Checked)
            {
                ...
                Result = BillingOperation.AddScanPCDDocument(JobId, DocumentId, strFilePath, LoggedInUser.glUserId, Convert.ToInt16(EnumBilltype.DraftInvoice));

                if (Result == 0)
                {

                    lblmessage.Text = "Document List Updated For Draft Invoice.";

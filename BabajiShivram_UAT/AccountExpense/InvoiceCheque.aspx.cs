...
public partial class AccountExpense_InvoiceCheque : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblTitle.Text = "Cheque/Invoice Submission";
        ...
        if (!Page.IsPostBack)
        {
            ...
            {
                int Chequeid = Convert.ToInt32(Session["ChequeId"]);
                DataSet dsChequeDetail = AccountExpense.GetIssuedCheckDetail(Chequeid);

                if(dsChequeDetail.Tables.Count>0)
                {
                    if(dsChequeDetail.Tables[0].Rows.Count>0)
                    {
                        int JobId = Convert.ToInt32(dsChequeDetail.Tables[0].Rows[0]["JobId"]);
                        string strJobNo = dsChequeDetail.Tables[0].Rows[0]["JobRefNo"].ToString();
                        ...
                        if (dsChequeDetail.Tables[0].Rows[0]["ModuleId"] != DBNull.Value)
                        {
                            hdnModuleId.Value = dsChequeDetail.Tables[0].Rows[0]["ModuleId"].ToString();
                        }
                        else
                        {
                            hdnModuleId.Value = "1";
            ...
            {
                Response.Redirect("PendingChequeInvoice.aspx");
            }
    ...
    protected void wzRequest_NextButtonClick(object sender, WizardNavigationEventArgs e)
    {
        ...
        else if (IndexId == 1) // Invoice Detail
        {
            ...
            else
            {
                ...
                BabajiGSTINDict.BabajiGSTIN.TryGetValue(txtCongisgneeGSTIN.Text.Trim(), out CheckBabajiGSTIN);
    ...
    protected void wzRequest_FinishButtonClick(object sender, WizardNavigationEventArgs e)
    {
        ...
        int ChequeId = 0;
        decimal decChequAmount = 0m;

        if(Session["ChequeId"] == null)
        {
            Response.Redirect("PendingChequeInvoice");
        }
        else
        {
            ChequeId = Convert.ToInt32(Session["ChequeId"]);
        }

        decimal.TryParse(txtChequeAmount.Text.Trim(), out decChequAmount);
        ...
        if (bIsInputValid)
        {
            ...
            string strVendorName, strVendorCode, strVendorGSTNo, strVendorPAN, strInvoiceNo, strRemark;
            bool IsAdvanceReceived = false, IsImmediatePayment = true;
            ...
            if (lblChequeDate.Text != "")
            {
                dtPaymentRequiredDate = Commonfunctions.CDateTime(lblChequeDate.Text.Trim());
            }
            if (lblChequeDate.Text != "")
            {
                dtPaymentDueDate = Commonfunctions.CDateTime(lblChequeDate.Text.Trim());
            }
            ...
            {
                int RequestId = AccountExpense.AddInvoiceDetail(JobId, strJobRefNO, ModuleID, BranchId, ExpenseTypeId,
                ...
                if (RequestId > 0)
                {
                    ...
                    AccountExpense.AddInvoiceStatus(RequestId, 120, "Auto Approved For Issued Cheque and  Submitted To Account", LoggedInUser.glUserId);

                    AccountExpense.UpdateBankChequeInvoiceNo(ChequeId, decChequAmount, RequestId, LoggedInUser.glUserId);
    ...
    protected void ddlExpenseType_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlExpenseType.SelectedValue == "1")         // Duty
        {

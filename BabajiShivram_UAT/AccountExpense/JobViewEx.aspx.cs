...
public partial class AccountExpense_JobViewEx : System.Web.UI.Page
{
    ...
    protected void PCDDocumentSqlDataSource_Selected(object sender, SqlDataSourceStatusEventArgs e)
    {
        if (e.Exception != null)
        {
            ...
            ErrorLog.LogToDatabase(Convert.ToInt32(Session["JobIdV"]), "PCDDocumentSqlDataSource_Selected:JobViewEx.aspx",
                PCDDocumentSqlDataSource.SelectCommand, e.Exception.Message, e.Exception, lblError.Text, LoggedInUser.glUserId);
            ErrorLog.SendMail("Error Reported In JobViewEx:Job Detail Form For PCDDocumentSqlDataSource_Selected", e.Exception);
        }
    }

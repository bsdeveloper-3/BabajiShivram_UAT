...
public partial class AccountExpense_InvoiceTransaction : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lblTitle.Text = "Bank Transaction";
        ...
        DataFilter1.FilterSessionID = "InvoiceTransaction.aspx";
    ...
    protected void gvDetail_RowCommand(Object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName.ToLower() == "select")
        {
            ...
            Session["InvoiceIdTrack"] = strInvoiceId;

            Response.Redirect("InvoiceDetail.aspx");
        }
        else if (e.CommandName.ToLower() == "checkstatus")
        {
            string strReferenceNo = Convert.ToString(e.CommandArgument);

            GetPaymentStatus(strReferenceNo);
        }
        else if (e.CommandName.ToLower() == "movetobanktransfer")
        {
            //MoveToBankTransfer
            int PaymentId = Convert.ToInt32(e.CommandArgument);

            int result = AccountExpense.UpdateFailedPaymentToBankTransfer(0, PaymentId, LoggedInUser.glUserId);
    ...
    public static void LIVEStatusCheck(int APITransactionID, int PaymentRequestId, string strRefNo, string strUTRNo, bool isAlreadyMailSent, int CurrentStatusId)
    {
        ...
        else if (objSuccess.Data != null)
        {
            ...
            if (StatusId == (int)EnumBankStatus.SettlementCompleted || StatusId == (int)EnumBankStatus.COMPLETED)
            {
                IsSuccess = 1;
            }
    ...
    private static bool SendPaymentConfirmationMail(int PaymentId)
    {
        ...
        if (IsMailSent == false)
        {
            ...
            string strCreatedByEmail = "Somnath.Kumbhar@BabajiShivram.com";
            ...
            try
            {
                ...
                try
                {
                    ...
                    bEmailSucces = EMail.SendMailCC("Somnath.Kumbhar@BabajiShivram.com", strCreatedByEmail, "", strSubject, EmailBody, "");
                }
    ...
    public void GetPaymentStatus(string strRefNo)
    {
        lblCHKStatus.Text = "";
        lblCHKStatusDate.Text = "";
        lblCHKStatusUTR.Text = "";
        lblCHKStatusUTRDate.Text = "";
        ...
        if (objError.Code != null)
        {
            ...
            lblCHKStatus.Text = objError.Code;
            lblCHKStatusMessage.Text = objError.Message;
        }
        else if (objSuccess.Meta != null)
        {
            lblCHKStatus.Text = objSuccess.Data.Status;

            if (objSuccess.Meta.ErrorSeverity != null)
            {
                lblCHKStatusMessage.Text = objSuccess.Meta.ErrorSeverity;
            }
            else if (objSuccess.Meta.ActionDescription != null)
            {
                lblCHKStatusMessage.Text = objSuccess.Meta.ActionDescription;
            }

            if (objSuccess.Data.StatusUpdateDateTime != DateTime.MinValue)
            {
                lblCHKStatusDate.Text = objSuccess.Data.StatusUpdateDateTime.ToString();
            }
            else
            {
                lblCHKStatusDate.Text = objSuccess.Data.CreationDateTime.ToString();
            }
            if (objSuccess.Data.Initiation.EndToEndIdentification != null)
            {
                lblCHKStatusUTR.Text = objSuccess.Data.Initiation.EndToEndIdentification;
            }
        }
        else if (objSuccess.Data != null)
        {
            lblCHKStatus.Text = objError.Code.ToString();
            lblCHKStatusDate.Text = objSuccess.Data.StatusUpdateDateTime.ToString();
            lblCHKStatusUTR.Text = objSuccess.Data.Initiation.EndToEndIdentification;
        }
        ...
        //else
        //{
        //    BankAPIHttpError.root httpError = new BankAPIHttpError.root();

        //    httpError = JsonConvert.DeserializeObject<BankAPIHttpError.root>(result);

        //    lblCHKStatus.Text =httpError.moreInformation;
    ...
    {
        string strFileName = "BankTransaction_" + DateTime.Now.ToString("dd/MM/yyyy hh:mm") + ".xls";

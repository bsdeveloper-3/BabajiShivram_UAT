...
public partial class FreightEnqModule_FreightEnq : System.Web.UI.Page
{
    string str_Message = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Convert.ToString(Session["VendorId"]) != null)
            {
                ...
                lblEnqRefNo.Text = DBOperations.GetNewFreightEnquiryNo();
    ...
    {
        Response.Redirect("FreightEnq.aspx");
    }
    ...
    protected void btnSave_OnClick(object sender, EventArgs e)
    {
        if (txtCustomer.Text == "")
        {
            ...
            strBuilder.Append(@"<div class='alert alert-mini alert-danger margin-bottom-30'><strong>Required!</strong>  Please enter Customer.</div>");
            dvError.InnerHtml = strBuilder.ToString();
            dvError.Visible = true;
        }
        else if (txtConsignee.Text == "")
        {
            StringBuilder strBuilder = new StringBuilder();
            strBuilder.Append(@"<div class='alert alert-mini alert-danger margin-bottom-30'><strong>Required!</strong>  Please enter Consignee.</div>");
            dvError.InnerHtml = strBuilder.ToString();
            dvError.Visible = true;
        }
        else if (txtPortOfDischarged.Text == "")
        {
            StringBuilder strBuilder = new StringBuilder();
            strBuilder.Append(@"<div class='alert alert-mini alert-danger margin-bottom-30'><strong>Required!</strong>  Please enter Port of Discharged.</div>");
            dvError.InnerHtml = strBuilder.ToString();
            dvError.Visible = true;
        }
        else if (txtPortLoading.Text == "")
        {
            StringBuilder strBuilder = new StringBuilder();
            strBuilder.Append(@"<div class='alert alert-mini alert-danger margin-bottom-30'><strong>Required!</strong>  Please enter Port of Loading.</div>");
            dvError.InnerHtml = strBuilder.ToString();
            dvError.Visible = true;
        }
        else if (ddlMode.SelectedValue == "2") // Mode is SEA
        {
            if (txtCont20.Text.ToString().Trim() == "" && txtCont40.Text.ToString().Trim() == "")
            {
                StringBuilder strBuilder = new StringBuilder();
                strBuilder.Append(@"<div class='alert alert-mini alert-danger margin-bottom-30'><strong>Required!</strong>  Please enter either of Container 20 or 40 (in optional tab)</div>");
                dvError.InnerHtml = strBuilder.ToString();
                dvError.Visible = true;
                txtCont20.Focus();
            }
            ...
            {
                string strEnquiryNo = lblEnqRefNo.Text.Trim();
                int IEC = 0, ShipmentType = 0, Pkgs = 0, Cont20 = 0, Cont40 = 0, IsDgGoods = 0, PortOfLoadingId = 0, PortOfDischargeId = 0;
                Decimal Quantity = 0, HsCode = 0, InvceValue = 0, Length = 0, Width = 0, Height = 0, Weight = 0;
                if (ddlIEC.SelectedValue == "1")
                    IEC = 1;
                else
                    IEC = 0;
                ShipmentType = Convert.ToInt32(ddlShipmentType.SelectedValue);
                if (txtQty.Text.Trim() != "")
                    Quantity = Convert.ToDecimal(txtQty.Text.Trim());
                if (txtHsCode.Text.Trim() != "")
                    HsCode = Convert.ToDecimal(txtHsCode.Text.Trim());
                if (txtInvcValue.Text.Trim() != "")
                    InvceValue = Convert.ToDecimal(txtInvcValue.Text.Trim());
                ...
                Pkgs = Convert.ToInt32(txtNoofPkg.Text.Trim());
                ...
                if (txtCont20.Text.Trim() != "")
                    Cont20 = Convert.ToInt32(txtCont20.Text.Trim());
                if (txtCont40.Text.Trim() != "")
                    Cont40 = Convert.ToInt32(txtCont40.Text.Trim());
                if (ddlDgHaz.SelectedValue == "1")
                    IsDgGoods = 1;
                else
                    IsDgGoods = 0;

                int result = DBOperations.AddNewEnquiry(lblEnqRefNo.Text.Trim(), Convert.ToInt32(ddlMode.SelectedValue), Convert.ToInt32(ddlTerms.SelectedValue), txtCustomer.Text.Trim(),
                    txtConsignee.Text.Trim(), IEC, txtShipper.Text.Trim(), txtShipperAddress.Text.Trim(), txtShipperPincode.Text.Trim(), PortOfLoadingId,
                    PortOfDischargeId, ShipmentType, txtCommodity.Text.Trim(), Quantity, Length, Width, Height, Weight, Pkgs, Cont20, Cont40, IsDgGoods,
                    txtProductLink.Text.Trim(), HsCode, InvceValue, txtDeliveryAdd.Text.Trim(), txtDeliveryAddPincode.Text.Trim(), Convert.ToInt32(Session["VendorId"]));
                if (result == 0)
                {
                    ResetControls();
                    StringBuilder strBuilder = new StringBuilder();
                    strBuilder.Append(@"<div class='alert alert-mini alert-success margin-bottom-30'><strong>Enquiry No. " + strEnquiryNo + "</strong> Successfully saved enquiry.</div>");
                    dvError.InnerHtml = strBuilder.ToString();
                    dvError.Visible = true;
                }
                else
                {
                    StringBuilder strBuilder = new StringBuilder();
                    strBuilder.Append(@"<div class='alert alert-mini alert-danger margin-bottom-30'><strong>Required!</strong> Error while inserting record.</div>");
                    dvError.InnerHtml = strBuilder.ToString();
                    dvError.Visible = true;
                }
            }
    ...
    //protected void btnAddNewCong_OnClick(object sender, EventArgs e)
    //{
    //    try
    //    {
    //        if (txtNewConsignee.Text != "")
    //        {
    ...
    //protected void btnAddNewCust_OnClick(object sender, EventArgs e)
    //{
    //    try
    //    {
    ...
    protected void ResetControls()
    {
        ...
        ddlIEC.SelectedValue = "0";
        txtShipper.Text = "";
        txtShipperAddress.Text = "";
        txtShipperPincode.Text = "";
        txtPortLoading.Text = "";
        txtPortOfDischarged.Text = "";
        ddlShipmentType.SelectedValue = "0";
        txtCommodity.Text = "";
        txtQty.Text = "";
        txtLength.Text = "";
        txtWidth.Text = "";
        ...
        txtCont20.Text = "";
        txtCont40.Text = "";
        ddlDgHaz.SelectedValue = "0";
        txtProductLink.Text = "";
        txtHsCode.Text = "";
        txtInvcValue.Text = "";
        txtDeliveryAdd.Text = "";
        txtDeliveryAddPincode.Text = "";
        dvError.Visible = false;
    }
        ...
        try
        {
            ...
            if (dsGetTotalMsgsRec != null && dsGetTotalMsgsRec.Tables.Count > 0)
            {
                if (dsGetTotalMsgsRec.Tables[0].Rows.Count > 0)
                {
                    ...
                    strBuilder.Append(@"<a href='FreightEnq.aspx?op=" + op + "'><i class='fa fa-envelope'></i> Inbox" +
                ...
                for (int i = 0; i < lstGetOnlineUsers.Count; i++)
                {
                    if (lstGetOnlineUsers[i].UserName.ToString() != "")
                    {
                        ...
                        strOnlineUsers.Append(@"<li class='media'><div class='media-body'><div class='media'>
                                                <a class='pull-left' href='FreightEnq.aspx?op=" + op + "&id=" + lid + "' style='margin-right: 0px'>");

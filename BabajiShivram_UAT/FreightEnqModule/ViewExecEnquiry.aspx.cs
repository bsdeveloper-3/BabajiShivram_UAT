...
public partial class FreightEnqModule_ViewExecEnquiry : System.Web.UI.Page
{
    string str_Message = "";
        ...
        try
        {
            if (loggedInUser.glRoleId.ToString() != "")
            {
                if (loggedInUser.glRoleId.ToString() == "53")
                {
                    ...
                    mList_FreightTracking = DBOperations.GetAlibabaTrackingDetails(5, "SelectAll", 0);
                ...
                {
                    List<AlibabaFreightTracking> mList_FreightTracking = new List<AlibabaFreightTracking>();
                    mList_FreightTracking = DBOperations.GetAlibabaTrackingDetails(5, "SelectUser", Convert.ToInt32(Session["VendorId"]));
                    if (mList_FreightTracking.Count > 0)
                    {
                        ViewState["FreightTracking"] = mList_FreightTracking;
                    }

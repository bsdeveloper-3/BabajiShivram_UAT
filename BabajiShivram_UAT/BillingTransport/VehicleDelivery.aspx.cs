...
public partial class BillingTransport_VehicleDelivery : System.Web.UI.Page
{
    ...
    protected void btnBack_OnClick(object sender, EventArgs e)
    {
        ...
        {
            if (Convert.ToString(Session["PageRedirect"]).Trim().ToLower() == "db")
                Response.Redirect("DashBoard.aspx");
            else
                Response.Redirect("UpdatedVehicle.aspx");
        }
        else
            Response.Redirect("UpdatedVehicle.aspx");
    }
    ...
    protected void lnkexport_Click(object sender, EventArgs e)
    {
        ...
        string strFileName = "DeliveredVehicleDetail_" + DateTime.Now.ToString("dd/MM/yyyy hh:mm tt") + ".xlsx";
        DataSet dsDeliveredVehicle = DBOperations.GetTPVehicleDetailReport(Convert.ToInt32(Session["JobId"]));
    ...
    private void ExportFunction(string header, string contentType, DataTable dtReport)
    {
        using (XLWorkbook wb = new XLWorkbook())
        {
            ...
            if (dtReport.Rows.Count > 0)
            {
                ...
                UnloadDate.Width = 18;
                var DetDays = workSheet.Column("M");
                DetDays.Width = 18;
                var DetChg = workSheet.Column("N");
                DetChg.Width = 18;
                var Freight = workSheet.Column("O");
                Freight.Width = 25;
                var WaraiChg = workSheet.Column("P");
                WaraiChg.Width = 18;
                var EmptyOffChg = workSheet.Column("Q");
                EmptyOffChg.Width = 26;
                var Tempo = workSheet.Column("R");
                Tempo.Width = 22;
                var EmptyRetDate = workSheet.Column("S");

...
using System.Text;
using Ionic.Zip;
public partial class BillingTransport_TransBillReceivedDetail : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("TransBillReceived.aspx");
        }

        if (!IsPostBack)
        {
            Label lblTitle = (Label)Page.Master.FindControl("lblTitle");
            lblTitle.Text = "Transport Bill Detail";
    ...
    {
        Response.Redirect("../AccountTransport/TransBillReceived.aspx");
    }
    ...
    protected void btnApproveBill_Click(object sender, EventArgs e)
    {
        if (hdnTransBillId.Value != "" && hdnTransBillId.Value != "0")
        {
            ...
            if (BillId > 0)
            {
                int result = DBOperations.AddBillReceivedDetail(BillId, 1, txtRemark.Text.Trim(), LoggedInUser.glUserId);
                
                if (result == 0)
                {
                    lblError.Text = "Bill detail Successfully Sent for HoD Approval!";
    ...
    protected void btnRejectBill_Click(object sender, EventArgs e)
    {
        if (hdnTransBillId.Value != "" && hdnTransBillId.Value != "0")
        {
            // Bill Rejected By Transport Team
            int BillId = Convert.ToInt32(hdnTransBillId.Value);
            if (BillId > 0)
            {
                int result = DBOperations.AddBillReceivedDetail(BillId, 51, txtRemark.Text.Trim(), LoggedInUser.glUserId);

                if (result == 0)
                {
                    lblError.Text = "Bill Rejected and detail Sent back To Transporter!";
                    lblError.CssClass = "success";
                    Session["Success"] = lblError.Text;

...
public partial class BillingTransport_BillingVehicle : System.Web.UI.Page
{
    ...
    private void JobDetailMS(int JobId)
    {
        ...
        lblTitle.Text = "Vehicle Detail - " + dsJobDetail.Tables[0].Rows[0]["JobRefNo"].ToString();
                ...
                if (result > 0)
                {
                    ...
                    if (gvJobDetail != null && gvJobDetail.Rows.Count > 0)
                    {
                        ...
                        if (ConsBillJob == 0)
                        {
                            ...
                            /******************** Show invoice copy upload option when last vehicle is to be updated  *************************/
                            DataSet dsGetVehicleDetail = new DataSet();
                            dsGetVehicleDetail = DBOperations.GetUpdatedVehicles(JobId, Convert.ToInt32(Session["VendorId"]));
                            if (dsGetVehicleDetail != null && dsGetVehicleDetail.Tables.Count > 0)
                            {
                                if (dsGetVehicleDetail.Tables[0].Rows.Count > 0)
                                {
                                    if (dsGetVehicleDetail.Tables[0].Rows[0]["Status"].ToString() != "" && dsGetVehicleDetail.Tables[0].Rows[0]["Status"].ToString() == "1")
                                    {
                                        hdnJobId.Value = JobId.ToString();
                                        hdnVehicleDeliveryId.Value = DeliveryId.ToString();
                ...
                if (result == 0)
                {
                    if (hdnTransportBillStatus.Value != "" && hdnTransportBillStatus.Value == "2")
                    {
                        ...
                        if (result2 == 0)
                        {
                            ModalPopupDocument.Hide();
                            ResetControls();
                            lberror.Text = "Successfully saved invoice copy. Job forwarded to transport department.";
                            lberror.CssClass = "success";
                            fsVehicleDetail.Visible = false;
                            btnBackToVehicleDetail.Visible = false;
                            fsEditDelivery.Visible = false;

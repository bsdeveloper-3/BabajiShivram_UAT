...
using System.IO;
using System.Text;
public partial class BillingTransport_VendorBillSubmission : System.Web.UI.Page
{
    private static Random _random = new Random();
    LoginClass LoggedInUser = new LoginClass();
    protected void Page_Load(object sender, EventArgs e)
    {
        if(LoggedInUser.glCompanyId == 0)
        {
            Response.Redirect("PendingVendorBill.aspx");
        }
    ...
    {
        Response.Redirect("JobWiseVehicleDetail.aspx");
    }
    ...
    protected void btnBillSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (isValidBill == true)
        {
            ...
            {
                string lblJobDIR = lblJobNo.Text.Trim().Replace("/","");
                lblJobDIR = lblJobDIR.Replace("-", "");
                filePath = lblJobDIR;

                fileName = UploadFiles(fuDocument, filePath + "\\");
                filePath = filePath + "\\"+ fileName;
            }

            TransporterID = LoggedInUser.glCompanyId;
            ...
            foreach (GridViewRow gr in GridViewVehicle.Rows)
            {
                ...
                if (gr.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (Session["TransReqId"] != null)
                    {
                        ...
                        //TextBox tdttst = (TextBox)gr.FindControl("txtRate");

                        //Rate        = Convert.ToInt32(((TextBox)gr.FindControl("txtRate")).Text);


                        Decimal.TryParse(((TextBox)gr.FindControl("txtRate")).Text.Trim(), out Rate);
                        ...
                        if (objLRCopy.HasFile)
                        {
                            string lblJobDIR = lblJobNo.Text.Trim().Replace("/", "");
                            lblJobDIR = lblJobDIR.Replace("-", "");
                            string LRfilePath = lblJobDIR;

                            LRCopy = UploadFiles(objLRCopy, LRfilePath + "\\");

                            LRCopy = LRfilePath + "\\" + LRCopy;
                        }
            ...
            if (resultRate == 0)
            {
                ...
                if (result_BillID > 0)
                {
                    // Bill Submitted By Transporter - 50
                    int result = DBOperations.AddTransApproveRejectBill(result_BillID, 50, Convert.ToDecimal(TotalAmount), 3);

                    if (result == 0)
                    {
                        // Send Bill to Transport Dept Approval

                        int AddHistory = DBOperations.AddTransBillApprovalHistory(result_BillID, 50, Remark, LoggedInUser.glUserId);
    ...
    protected void btnCancelBill_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("RejectedVendorBill.aspx");
    ...
    protected void chkGSTBill_CheckedChanged(object sender, EventArgs e)
    {
    ...
    private DataTable CreateDataviewCharges()
    {
        // Create a DataTable
        DataTable table = new DataTable("Charges");
        // Add columns to the DataTable

        table.Columns.Add("Charge Type", typeof(string));
        table.Columns.Add("Rate", typeof(decimal));
        table.Columns.Add("Unit", typeof(string));
        table.Columns.Add("Tax Percent", typeof(decimal));
        table.Columns.Add("Total Amount", typeof(decimal));
        // Add rows to the DataTable

        table.Rows.Add("FreightRate", 0m, "", 0m, 0m);
        table.Rows.Add("Detention", 0m, "", 0m, 0m);
        table.Rows.Add("Varai", 0m, "", 0m, 0m);
        table.Rows.Add("Empty", 0m, "", 0m, 0m);
        table.Rows.Add("Toll", 0m, "", 0m, 0m);
        table.Rows.Add("Union", 0m, "", 0m, 0m);
    ...
    {
        if (e.CommandName.ToUpper() == "ADDGSTBILL")
        {
            int VehicleRateID = Int32.Parse(e.CommandArgument.ToString());
            hdnRateId.Value = VehicleRateID.ToString();

            GridViewRow gvrow = (GridViewRow)((LinkButton)e.CommandSource).NamingContainer;
            Label objVehicleNo = (Label)gvrow.FindControl("lblVehicleNo");

            lblPopupMessage.Text = objVehicleNo.Text;
            lblVehicleNo.Text = objVehicleNo.Text;

            DataRateDetails.SelectParameters[0].DefaultValue = VehicleRateID.ToString();
            DataRateDetails.DataBind();
    ...
    //private void BindData() { List<Charge> charges = new List<Charge> 
    //{ 
    //    //new Charge { ChargeType = "FreightRate", Rate = 100.00m, Unit = "kg", TaxPercent = 0.05m, TotalAmount = 105.00m }, 
    //    //new Charge { ChargeType = "Detention", Rate = 200.00m, Unit = "hr", TaxPercent = 0.10m, TotalAmount = 220.00m }, 
    //    //new Charge { ChargeType = "Varai", Rate = 150.00m, Unit = "kg", TaxPercent = 0.08m, TotalAmount = 162.00m }, 
    //    //new Charge { ChargeType = "Empty", Rate = 50.00m, Unit = "unit", TaxPercent = 0.02m, TotalAmount = 51.00m }, 
    //    //new Charge { ChargeType = "Toll", Rate = 75.00m, Unit = "trip", TaxPercent = 0.05m, TotalAmount = 78.75m }, 
    //    //new Charge { ChargeType = "Union", Rate = 120.00m, Unit = "day", TaxPercent = 0.12m, TotalAmount = 134.40m } }; 

    //    //GridView1.DataSource = charges; 
    //    //GridView1.DataBind(); 
    //}
    ...
    protected void btnSaveVehicleRate_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in gvRateDetails.Rows)
        {
            int ChargeTypeID = Convert.ToInt32(gvRateDetails.DataKeys[row.RowIndex].Value);

            Label objChargeType = (Label)row.FindControl("lblChargeType");
            //Label objChargeType = (Label)row.Cells[0].FindControl("lblChargeType");
            //CheckBox chk1 = (CheckBox)gvr1.Cells[1].FindControl("chk1");

            TextBox objtxtCharges = (TextBox)row.FindControl("txtCharges");
            DropDownList obddTaxType = (DropDownList)row.FindControl("ddTaxType");
            TextBox objTaxPercent = (TextBox)row.FindControl("txtTaxPercent");
            TextBox objTaxAmount = (TextBox)row.FindControl("txtTaxAmount");
            TextBox objTotalAmount = (TextBox)row.FindControl("txtTotalAmount");

            string chargeType = objChargeType.Text;
            int taxType = 0;
            int Freightcharges = 0;
            int taxPercent = 0;
            int taxAmount = 0;
            int totalCharges = 0;

            Int32.TryParse(objtxtCharges.Text.Trim(), out Freightcharges);

            taxType = Convert.ToInt32(obddTaxType.SelectedValue);
            Int32.TryParse(objTaxPercent.Text.Trim(), out taxPercent);
            Int32.TryParse(objTaxAmount.Text, out taxAmount);
            Int32.TryParse(objTotalAmount.Text, out totalCharges);

            string taxTypeName = obddTaxType.SelectedItem.Text;

            Charge charge = chargesList.Find(c => c.ChargeTypeId == ChargeTypeID);

            if (charge != null)
            {
                charge.ChargeTypeId = ChargeTypeID;
                charge.ChargeType = chargeType;
                charge.Charges = Freightcharges;
                charge.TaxType = taxType;
                charge.TaxPercent = taxPercent;
                charge.TaxAmount = taxAmount;
                charge.TotalCharges = totalCharges;
                charge.TaxTypeName = taxTypeName;
            }
            else
            {
                Charge NewCharge = new Charge();

                NewCharge.ChargeTypeId = ChargeTypeID;
                NewCharge.ChargeType = chargeType;
                NewCharge.Charges = Freightcharges;
                NewCharge.TaxType = taxType;
                NewCharge.TaxPercent = taxPercent;
                NewCharge.TaxAmount = taxAmount;
                NewCharge.TotalCharges = totalCharges;
                NewCharge.TaxTypeName = taxTypeName;
        ...
        if (AddVehicleCharges(RateID, chargesList))
        {
            GridViewVehicle.DataBind();

            lblError.Text = "Charge details updated successfully!";
            lblError.CssClass = "success";
            ModalPopupExtender1.Hide();
        }
        else
        {
            lblPopupMessage.Text = "System Error! Please Try after sometime";
            lblPopupMessage.CssClass = "errorMsg";
        }
    }

    private bool AddVehicleCharges(int RateId, List<Charge> RateDetail)
    {
        bool isSuccess = true;

        foreach (Charge charge in RateDetail)
        {
            int Result = TransOperation.AddTransporterBillDetail(RateId, charge.ChargeTypeId, charge.Charges, charge.TaxType, charge.TaxPercent, charge.TaxAmount, charge.TotalCharges, LoggedInUser.glUserId);

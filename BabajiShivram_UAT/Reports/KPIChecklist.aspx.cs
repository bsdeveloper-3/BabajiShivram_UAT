...
public partial class Reports_KPIChecklist : System.Web.UI.Page
{
    ...
    private void CreateBOEChartPIE(DataView view)
    {
        ...
        DataTable ChecklistData = GroupBySUM("Name", "ProductCount", view.ToTable());

        ChecklistData.DefaultView.Sort = "Count desc";

        ChecklistData = ChecklistData.DefaultView.ToTable();

        /********************* ASP BOE BAR Chart Start *******************************

        cTestChart.Series["Testing"].ChartType = SeriesChartType.Bar;
        int i=0;
        foreach (DataRow dr in ChecklistData.Rows)
        { 
            int y = (int) dr[i];
        ...
        if (ChecklistData.Rows.Count > 0)
        {
            ...
            string[] XPointMember = new string[ChecklistData.Rows.Count];
            int[] YPointMember = new int[ChecklistData.Rows.Count];

            //for (int count = 0; count < ChecklistData.Rows.Count; count++)
            int count = 0;
            foreach (DataRow dr in ChecklistData.Rows)
            {
            ...
            //Enabled 3D  
            BOEKPIChartPIE.ChartAreas["ChecklistKPIArea"].Area3DStyle.Enable3D = true;
            ...
            BOEKPIChartPIE.Legends["ChecklistKPILegends"].CellColumns.Add(firstColumn);
            ...
            BOEKPIChartPIE.Legends["ChecklistKPILegends"].CellColumns.Add(secondColumn);
            ...
            BOEKPIChartPIE.Legends["ChecklistKPILegends"].CellColumns.Add(thirdColumn);
            ...
            BOEKPIChartPIE.Legends["ChecklistKPILegends"].CellColumns.Add(fourthColumn);

            // Add header separator of type line
            BOEKPIChartPIE.Legends["ChecklistKPILegends"].HeaderSeparator = LegendSeparatorStyle.Line;
            BOEKPIChartPIE.Legends["ChecklistKPILegends"].HeaderSeparatorColor = Color.FromArgb(64, 64, 64, 64);

            // Add item column separator of type line
            BOEKPIChartPIE.Legends["ChecklistKPILegends"].ItemColumnSeparator = LegendSeparatorStyle.Line;
            BOEKPIChartPIE.Legends["ChecklistKPILegends"].ItemColumnSeparatorColor = Color.FromArgb(64, 64, 64, 64);
    ...
    private void CreateBOEChartBAR(DataView view)
    {
        ...
        string[] x = new string[ChecklistData.Rows.Count];
        decimal[] y = new decimal[ChecklistData.Rows.Count];
        for (int i = 0; i < ChecklistData.Rows.Count; i++)
        {
            x[i] = ChecklistData.Rows[i][0].ToString();
            y[i] = Convert.ToInt32(ChecklistData.Rows[i][1]);
        }
        ...
        BarChart1.ChartTitle = "Checklist KPI - PRODUCT";
        ...
        BarChart2.ChartTitle = "Checklist KPI - JOB";

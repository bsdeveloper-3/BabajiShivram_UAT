...
using System.Data;
using System.Text;
public partial class AccountTransport_BillSubmissionEdit : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (LoggedInUser.glUserId == 0 || Session["TransPayId"] == null)
        {
            Response.Redirect("PendingTransRejected.aspx");
        }
        ...
        if (!IsPostBack)
        {
            ...
            {
                FundRequestDetail(Convert.ToInt32(Session["TransPayId"]));
                //TruckRequestDetail(Convert.ToInt32(hdnTransReqId.Value));
                JobDetailMS(Convert.ToInt32(hdnJobId.Value));
            }
            else
            {
                hdnTransReqId.Value = "0";
    ...
    private void FundRequestDetail(int PayRequestId)
    {
        ...
        {
            hdnTransReqId.Value     = dvFundDetail.Table.Rows[0]["TransReqId"].ToString();
            hdnTransporterId.Value  = dvFundDetail.Table.Rows[0]["VendorId"].ToString();
            hdnJobId.Value          = dvFundDetail.Table.Rows[0]["JobId"].ToString();

            txtBillNo.Text = dvFundDetail.Table.Rows[0]["InvoiceNo"].ToString();
            txtTotalAmount.Text  = dvFundDetail.Table.Rows[0]["Amount"].ToString();

            if (dvFundDetail.Table.Rows[0]["InvoiceDate"] != DBNull.Value)
            {
                txtBillDate.Text = Convert.ToDateTime(dvFundDetail.Table.Rows[0]["InvoiceDate"]).ToString("dd/MM/yyyy");
            }
        }
    }
    ...
    //    {
    //        JobId = Convert.ToInt32(dvDetail.Table.Rows[0]["JobId"].ToString());
    ...
    protected void btnBillSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (isValidBill == true)
        {
            ...
            bool IsValid = true;
            int PaymentId =0, TransReqId = 0, TransporterID = 0;
            ...
            PaymentId  = Convert.ToInt32(Session["TransPayId"]);
            TransReqId = Convert.ToInt32(hdnTransReqId.Value);
            TransporterID = Convert.ToInt32(hdnTransporterId.Value);
            ...
            int TDSResult = DBOperations.AddTransPayTDS(TransReqId, tdsApplicable, "", 0, 0, LoggedInUser.glUserId);
            ...
            foreach (GridViewRow gr in GridViewVehicle.Rows)
            {
                ...
                if (gr.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (TransReqId > 0 && TransporterID > 0)
                    {
                        ...
                        {
                            lblError.Text = "Rate detail does not exists." + TransReqId.ToString() +"-" + RateId.ToString() + "-" + VehicleNo.ToString();
                            lblError.CssClass = "errorMsg";
                        }
                        else
                        {
                            lblError.Text = "System error. Please try again later." + TransReqId.ToString() + "-" + RateId.ToString() + "-" + VehicleNo.ToString();
                            lblError.CssClass = "errorMsg";
                        }
                    }
                }//END_IF
            ...
            if (resultRate == 0)
            {
                // Update Payment Request Detail and send Bill For Transport HOD Approval

                int result_BillID = DBOperations.UpdateTransBillDetail(PaymentId, TransReqId, TransporterID, BillNumber, BillDate, FreightRateTotal.ToString(),
                    DetentionTotal.ToString(), VaraiTotal.ToString(), EmptyTotal.ToString(), TollTotal.ToString(), OtherTotal.ToString(), 
                    TotalAmount.ToString(),txtJustification.Text.Trim(),false,0,fileName, filePath, LoggedInUser.glUserId);

                if (result_BillID > 0)
                {
                    lblError.Text = "Rate Details Updated";
                    lblError.CssClass = "errorMsg";
                    // Send Bill To A/C Department For Mgmt Approval

                    int result_Account = DBOperations.UpdateTransBillPaymentRequest(PaymentId, result_BillID, TransReqId, TransporterID, txtJustification.Text.Trim(), LoggedInUser.glUserId);

                    if (result_Account > 10)
                    {
                        lblError.Text = "Bill Detail Send to Mgmt Approval!";
                        lblError.CssClass = "success";
                    ...
                    else if (result_Account == 1)
                    {
                        lblError.Text = "Bill Detail Not Found! Please Try After Sometime.";
                        lblError.CssClass = "errorMsg";
                    }
                    else if (result_Account == 2)
                    {
                        lblError.Text = "Payment Request Error! Please Try After Sometime.";
                        lblError.CssClass = "errorMsg";
                    }
                    else if (result_Account == 3)
                    {
                        lblError.Text = "Job Detail Not Found! Please Try After Sometime.";
                        lblError.CssClass = "errorMsg";
                    }
                    else if (result_Account == 4)
                    {
                        lblError.Text = "Job Detail Not Found! Please Try After Sometime.";
                        lblError.CssClass = "errorMsg";
                    }
                ...
                else if (result_BillID == -4)
                {
                    lblError.Text = "Bill Already Exists For Transporter!";
                    lblError.CssClass = "errorMsg";
                }
                else if (result_BillID == -5)
                {
                    lblError.Text = "Transporter Bill Details Not Found!";
                    lblError.CssClass = "errorMsg";
                }
                else
                {
            ...
            {
                lblError.Text = "Rate detail does not exists." + TransReqId.ToString();
                lblError.CssClass = "errorMsg";
            }
        }
    ...
    private bool ValidateBill()
    {
        ...
        {
            isValidAmount = false;
            lblError.Text = "Bill Amount and Vehcile Detail Amount Not Matched. Vehicle Total Rs:" + VehicleTotal.ToString();
            lblError.CssClass = "errorMsg";
        }
        else if (TotalAmount == 0 || VehicleTotal == 0)
        {
            isValidAmount = false;
            lblError.Text = "Bill Amount and Vehcile Detail Amount Not Matched.";
            lblError.CssClass = "errorMsg";
        }

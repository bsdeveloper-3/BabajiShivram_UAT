...
public partial class FreightOperation_ExAwaitingBooking : System.Web.UI.Page
{
    ...
    protected void gvFreight_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName.ToLower() == "navigate")
        {
            ...
            string strEnqId = commandArgs[0].Trim();
            string strlType = commandArgs[1].Trim();

            Session["EnqId"] = strEnqId;
            Session["lType"] = strlType;
            //string[] commandArgs = e.CommandArgument.ToString().Split(new char[] { ',' });
            ...
            {
                Response.Redirect("ExBookingDetail.aspx");
            }
        }
    }
    ...
    {
        string strFileName = "AwaitingFreightBooking_" + DateTime.Now.ToString("dd/MM/yyyy hh:mm tt") + ".xls";

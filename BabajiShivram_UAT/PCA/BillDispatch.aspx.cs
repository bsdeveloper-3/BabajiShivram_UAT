...
using System.Data;
using MyPacco.API;
public partial class PCA_BillDispatch : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Session["BillJobId"] != null)
            {
                hdnJobId.Value = Session["BillJobId"].ToString();

                DataSet dsJobClientStatus = BillingOperation.GetBillDispatchCondition(Convert.ToInt32(hdnJobId.Value));

                if(dsJobClientStatus.Tables.Count > 0)
                {
                    if (dsJobClientStatus.Tables[0].Rows.Count > 0)
                    {
                        if(dsJobClientStatus.Tables[0].Rows[0]["DispatchStatus"] != DBNull.Value)
                        {
                            int intDispatchStatus = Convert.ToInt32(dsJobClientStatus.Tables[0].Rows[0]["DispatchStatus"]);

                            if(intDispatchStatus == 1)
                            {
                                intDispatchStatus = 0; // Required
                            }

                            rblPhysicalBillRequired.SelectedValue = intDispatchStatus.ToString();
                        }
                        if (dsJobClientStatus.Tables[0].Rows[0]["EBillStatus"] != DBNull.Value)
                        {
                            int intEBillStatus = Convert.ToInt32(dsJobClientStatus.Tables[0].Rows[0]["EBillStatus"]);

                            if (intEBillStatus == 1)
                            {
                                intEBillStatus = 0; // Required
                            }

                            rblEBillRequired.SelectedValue = intEBillStatus.ToString();
                        }
                        if (dsJobClientStatus.Tables[0].Rows[0]["PortalStatus"] != DBNull.Value)
                        {
                            int intPortalStatus = Convert.ToInt32(dsJobClientStatus.Tables[0].Rows[0]["PortalStatus"]);

                            if (intPortalStatus == 1)
                            {
                                intPortalStatus = 0; // Required
                            }

                            rblBillClientPortal.SelectedValue = intPortalStatus.ToString();
                        }
                    }
                }
            ...
            DataSourceBillJob.SelectParameters[0].DefaultValue = hdnJobId.Value; // "200117"; //346660
            DataSourceBillJob.SelectParameters[1].DefaultValue = "1";
            DataSourceBillJob.DataBind();
            gvJobDetail.DataBind();

            // Dispatch History

            SqlDataSourceHistory.SelectParameters[0].DefaultValue = hdnJobId.Value; // "200117"; //346660
            SqlDataSourceHistory.SelectParameters[1].DefaultValue = "1";
    ...
    protected void gvJobDetail_RowCommand(Object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName.ToLower() == "removebill")
        {
            int BillId = Convert.ToInt32(e.CommandArgument);

            int result = BillingOperation.DeleteBillDocById(BillId, LoggedInUser.glUserId);

            if (result == 0)
            {
                lblMessage.Text = "Bill Document Removed!";
            ...
            {
                lblMessage.Text = "Bill Document Not Found!"+ BillId.ToString();
                lblMessage.CssClass = "errorMsg";
    ...
    protected void btnUpdateBillStatus_Click(object sender, EventArgs e)
    {
        ...
        int PhysicalBillStatus = 0; // Pending Dispatch
        int EBillStatus = 0;// Pending E-Bill
        int ClientPortalStatus = 0; // // Pending Client Portal Upload

        if (rblPhysicalBillRequired.SelectedIndex == -1)
        {
            lblMessage.Text = "Please Confirm Physical Dispatch Required!";
            lblMessage.CssClass = "errorMsg";
            return;
        }
        else
        {
            PhysicalBillStatus = Convert.ToInt32(rblPhysicalBillRequired.SelectedValue);
        }
        if (rblEBillRequired.SelectedIndex == -1)
        {
            lblMessage.Text = "Please Confirm E-Bill Email Required!";
            lblMessage.CssClass = "errorMsg";
            return;
        }
        else
        {
            EBillStatus = Convert.ToInt32(rblEBillRequired.SelectedValue);
        }

        if (rblBillClientPortal.SelectedIndex == -1)
        {
            lblMessage.Text = "Please Confirm Client Portal Update Required!";
            lblMessage.CssClass = "errorMsg";
            return;
        }
        else
        {
            ClientPortalStatus = Convert.ToInt32(rblBillClientPortal.SelectedValue);
        }

        if (PhysicalBillStatus == 2 && EBillStatus == 2 && ClientPortalStatus == 2 )
        {
        ...
        else
        {
            ...
            foreach (GridViewRow rw in gvJobDetail.Rows)
            {
                ...
                if (chk.Checked)
                {
                    result = BillingOperation.AddBillDispatchCondition(JobId, BillId, PhysicalBillStatus, EBillStatus, ClientPortalStatus, 1,LoggedInUser.glUserId);
                }
            }
    ...
    protected void gvJobHistory_RowCommand(Object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName.ToLower() == "viewcovernote")
        {
            int BillId = Convert.ToInt32(e.CommandArgument);

            DataSet dsDocDetail = BillingOperation.GetBillCoverDocByBillId(BillId);
    ...
    private void GenerateEmailDraft(DataTable dtDispatchBill)
    {
        ...
        DataSet dsCustomerEmail = BillingOperation.GetBillDispatchEmail(JobId, "");
    ...
    private void BindAttachBillDoc(List<BJVBill> lstBillList)
    {
        ...
        foreach (DataRow rows in dtDoc.Rows)
        {
            ...
            //lblMessage.Text = lblMessage.Text + "$" + ServerPath;
    ...
    {
        int JobId = Convert.ToInt32(hdnJobId.Value);
        string strJobIdList = JobId.ToString();
        ...
        else
        {
            ...
            {
                int EBillResult  = BillingOperation.updateBillingEBillList(strJobIdList, DateTime.Now, "", LoggedInUser.glUserId);
    ...
    {        
        ...
        else
        {
            ...
            //bEmailSucces = EMail.SendMailMultiAttach("Query.Billing@BabajiShivram.com", strCustomerEmail, strCCEmail, strSubject, MessageBody, lstFilePath);
    ...
    protected void MyPaccoAWBGeneration()
    {
        ...
        if(ViewState["AWBJobid"].ToString() != "")
        {
            ...
            if (dsJobAWB.Tables.Count > 0)
            {
                ...
                {
                    lblMessage.Text = "Dispatch Plant";
    ...
    protected void btnClientPortalUpload_Click(object sender, EventArgs e)
    {
        ...
        lblMessage.Text = "";
        hdnBillList.Value = "";
        txtClientPortalDate.Text = "";
        txtClientPortalRefNo.Text = "";
        ...
        foreach (GridViewRow rw in gvJobDetail.Rows)
        {
            ...
            if (chk.Checked)
            {
                hdnBillList.Value = hdnBillList.Value + BillId.ToString() + ",";
            }
        }

        if (hdnBillList.Value == "")
        {
            lblMessage.Text = "Please Select Bill Number!";
            lblMessage.CssClass = "errorMsg";
        }
        else
        {
            ClientUploadModalPopup.Show();
        }
    }

    protected void btnSaveClientDate_Click(object sender, EventArgs e)
    {
        DateTime dtClientUploadDate = DateTime.MinValue;
        string strClientPortalRefNo = "";

        if (txtClientPortalDate.Text.Trim() != "" && txtClientPortalRefNo.Text.Trim() != "")
        {
            
            String[] lstBillList = hdnBillList.Value.Split(',');

            strClientPortalRefNo = txtClientPortalRefNo.Text.Trim();
            dtClientUploadDate = Commonfunctions.CDateTime(txtClientPortalDate.Text.Trim());

            int DateDiff = DateTime.Compare(DateTime.Now.Date, dtClientUploadDate);

            if (DateDiff < 0)
            {
                lblMessage.Text = "Please Enter Valid Bill Upload Date!";
                lblMessage.CssClass = "errorMsg";
                ClientUploadModalPopup.Show();
            }
            else
            {
                ...
                foreach (string strBillId in lstBillList)
                {
                    if (strBillId != "")
                    {
                        result = BillingOperation.AddBillClientPortalDate(0, Convert.ToInt32(strBillId), dtClientUploadDate, strClientPortalRefNo, LoggedInUser.glUserId);
                    }
                }
                ...
                if(result == 0)
                {
                    lblMessage.Text = "Detail Updated Successfully!";
                    lblMessage.CssClass = "success";
                    ClientUploadModalPopup.Hide();

                    gvJobDetail.DataBind();
                }
                else if (result == 2)
                {
                    lblMessage.Text = "Detail Already Updated!";
                    lblMessage.CssClass = "errorMsg";
                    ClientUploadModalPopup.Hide();
                }
                else
    ...
    protected void btnApprove_Click(object sender, EventArgs e)
    {
        ...
        result = BillingOperation.AddBillDispatchComplete(JobId, bApprove,"",1, LoggedInUser.glUserId);
        ...
        {
            lblMessage.Text = "Dispatch Cover Letter Not Generated! Please Generate Cover Letter!";
            lblMessage.CssClass = "errorMsg";
        }
        else if (result == 3)
        {
            lblMessage.Text = "Bill Upload Pending! Please Upload All Bills.";
            lblMessage.CssClass = "errorMsg";
        }
        else if (result == 4)
        {
            lblMessage.Text = "Client Requirement Update Pending.";
            lblMessage.CssClass = "errorMsg";
        }
        else
        {
    ...
    //else
    //{
    //    lblMessage.Text = "Please Generate Covering Letter First!.";
    //    lblMessage.CssClass = "errorMsg";
    //}

    #endregion

    #region E-Bill Backlog Update
    protected void btnBacklogEmail_Click(object sender, EventArgs e)
    {
        int JobId = Convert.ToInt32(hdnJobId.Value);
        string strJobIdList = JobId.ToString();
        ...
        else
        {
            ...
            if (bEMailSucess == true)
            {
                int EBillResult = BillingOperation.updateBillingEBillList(strJobIdList, DateTime.Now, "EBill Backlog", LoggedInUser.glUserId);
                ...
                if (Result == 0)
                {
                    lblMessage.Text = "E-Bill Backlog Updated Successfully!";
                    lblMessage.CssClass = "success";
                }
                else if (Result == 1)
                {
            ...
            {
                lblMessage.Text = "Email Backlog Failed! Please Enter Comma-Seperated Email Addresses";
                lblMessage.CssClass = "errorMsg";
            }
        }
